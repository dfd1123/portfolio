...
class NiceCheck {
    public function NiceCheck_main()
    {
    	...
    	$sitecode = "BP645";				// NICE로부터 부여받은 사이트 코드
    	$sitepasswd = "7UvN8FAZ0diD";			// NICE로부터 부여받은 사이트 패스워드
		...
		/*
		$returnurl = env('APP_URL')."/nicecheck/checkplus_success";	// 성공시 이동될 URL 보안인증일경우 URL
		$errorurl = env('APP_URL')."/nicecheck/checkplus_fail";		// 실패시 이동될 URL 보안인증일경우 URL
		*/

		$returnurl = env('APP_URL')."/auth/checkplus_success";	// 성공시 이동될 URL
		$errorurl = env('APP_URL')."/auth/checkplus_fail";		// 실패시 이동될 URL
	...
	public function NiceCheck_success()
    {
	    ...
	    if ($enc_data != "") {
	        ...
	        if ($plaindata == -1){
				...
				$returnMsg = $mobileno;
				return ["mobileno" => $mobileno, "name" => $name, "name_utf8" => $name_utf8, "returnMsg" => "성공"];
	        }
	    }
	}
