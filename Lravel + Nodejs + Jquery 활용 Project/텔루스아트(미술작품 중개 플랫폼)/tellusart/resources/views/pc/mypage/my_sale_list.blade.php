...
<div id="container">
	...
	<div class="orderbox">
		...
		<div class="period">
			<form id="my_sale_list" method="get" action="{{route('mypage.my_sale_list')}}">
				<ul>
			...
			<div class="buy_goods_list">
				<div class="table_list">
					<table class="list_cart">
						...
						@forelse($sales as $sale)
							<tr class="pop_parent" >
								...
								<td>
									...
									{{($sale->delivery->delivery_company != NULL)? Facades\App\Classes\SweetTracker::companyname($sale->delivery->delivery_company):"-"}}<br>
									{{($sale->delivery->send_post_num != NULL)? $sale->delivery->send_post_num:""}}
									@endif
								</td>
								...
								<td class="t_layout web">
									<div class="cart_m_bt">
										<span class="w100">
											...
											@elseif($sale->order_state == 1)
												@if($sale->order_cancel == 0)
													<button type="button" class="insert_delivery m_btn" data-id="{{$sale->id}}" title="작품발송">작품발송</button>
												@elseif($sale->order_cancel == 1)
													주문취소중
													<button type="button" class="view_cancel_reason m_btn" data-id="{{$sale->id}}" title="사유보기">사유보기</button>
												@endif
											@elseif($sale->order_state == 2)
												@if($sale->order_cancel == 0)
													<button type="button" class="view_delivery m_btn" data-id="{{$sale->id}}" title="배송내역보기">배송내역보기</button>
												@elseif($sale->order_cancel == 1)
													주문취소중
													<button type="button" class="view_cancel_reason m_btn" data-id="{{$sale->id}}" title="사유보기">사유보기</button>
												@endif
						...
						@empty
							<tr>
								<td colspan="6">검색되는 판매 내역이 없습니다.</td>
							</tr>
						@endforelse
		...
		<div class="jw_modal_content">
			...
			<div class="jw_modal_bd">
				<div class="content_box">
					...
					<input type="button" class="on" value="확인">
				</div>
			</div>
		</div>
	</div>
...
<div id="insert_delivery_modal"  class="jw_modal_wrap hidden">
	<div class="jw_overlay"></div>
	<div class="jw_modal_content_wrap like_cux">
		<div class="jw_modal_content">
			<div class="jw_modal_hd">
			...
			<div class="jw_modal_bd">
				<form method="get" action="{{route('mypage.insert_delivery')}}">
					...
					<div class="content_box">
						...
						<input type="text" name="send_post_num" required/>
						<input type="submit" class="on" value="제출">
					</div>
				</form>
			</div>
		</div>
		...
		<div class="jw_modal_content">
			...
			<div class="jw_modal_bd">
				<form method="get" action="{{route('mypage.insert_delivery')}}">
					...
					<div class="content_box">
						<h5><i class="fal fa-chevron-circle-right"></i>택배회사</h5>
						<select name="delivery_company" id="modal_delivery_company" class="select">
						@foreach($delivery_companys['Company'] as $delivery_company)
							<option value="{{ $delivery_company['Code'] }}">{{ $delivery_company['Name'] }}</option>
						@endforeach
						</select>
						<h5><i class="fal fa-chevron-circle-right"></i>송장번호</h5>
						<input type="text" name="send_post_num" id="modal_send_post_num" required/>
...
<style>
	input[type='button'].on, input[type='submit'].on {
		display: block;
		margin: 0 auto;
		width: 100%;
		font-size: 15px;
