...
const enableTryState = false; // infura와 통신 실패 체크하며 진행. 실패 시 중지
...
module.exports = {
    ...
    try {
      switch (withdrawRequest.request_status) {
        case 'withdraw_requested': {
          console.log(
            `출금요청 추가 -> 출금종류: ${withdrawRequest.coin_kind}, 출금유저: ${withdrawRequest.request_user_id}, 출금주소: ${withdrawRequest.request_address}, 출금액수: ${withdrawRequest.request_amount}`,
          );
  ...
  async processWithdrawFromUserEthRequest(withdrawRequest) {
    try {
      switch (withdrawRequest.request_status) {
        case 'withdraw_requested': {
          console.log(
          ...
          if (enableTryState) {
            console.info('사용자 주소에서 이더 출금 요청 시작 처리');
            await call.updateRequestValue({
              id: withdrawRequest.id,
              requestStatus: 'withdraw_eth_request_try',
            });
          ...
          // 사용자 주소 정보 가져오기
          const user = await call.getAccountFromUserId({
            userId: withdrawRequest.request_user_id,
          });

          if (user === null) {
            console.info('알수없는 사용자에게서 출금 처리 불가');
            await call.updateRequestValue({
              id: withdrawRequest.id,
              inProgress: '3',
              pendingTx: null,
              requestStatus: 'withdraw_user_check_fail',
            });
            return false;
          }

          console.info('사용자 주소에서 요청 주소로 출금 처리 (수수료 포함)');
          const txHash = await tx.sendEthTransaction(
            user.address,
            withdrawRequest.request_address,
            withdrawRequest.request_amount,
      ...
      const knownTransactionErrorMsg = 'insufficient funds for';
      if (err.message.startsWith(knownTransactionErrorMsg)) {
        console.info('사용자 이더 잔액 부족');
        await call.updateRequestValue({
          id: withdrawRequest.id,
          inProgress: '3',
          pendingTx: null,
          requestStatus: 'insufficient_funds_eth_user_fail',
        });
        return false;
      }
      throw err;
  ...
  async processWithdrawFromUserTokenRequest(withdrawRequest) {
    try {
      switch (withdrawRequest.request_status) {
        case 'withdraw_requested': {
          console.log(
          ...
          if (enableTryState) {
            console.info('사용자 주소에서 토큰 출금 요청 시작 처리');
            await call.updateRequestValue({
              id: withdrawRequest.id,
              requestStatus: 'withdraw_token_request_try',
            });
          ...
          console.info('사용자 주소에서 요청 주소로 출금 처리');
          const txHash = await tx.sendErcTransaction(
            user.address,
            withdrawRequest.request_address,
            withdrawRequest.request_amount,
      ...
      const knownTransactionErrorMsg = 'insufficient funds for';
      if (err.message.startsWith(knownTransactionErrorMsg)) {
        console.info('사용자 토큰 잔액 부족');
        await call.updateRequestValue({
          id: withdrawRequest.id,
          inProgress: '3',
          pendingTx: null,
          requestStatus: 'insufficient_funds_token_user_fail',
        });
        return false;
      }
      throw err;
      ...
      switch (depositRequest.request_status) {
        ...
        case 'deposit_token_request_get_eth': {
          ...
          if (tokenTransaction === null) {
            throw new Error('트랜잭션 동기화 대기');
          }

          if (tokenTransactionReceipt === null) {
            throw new Error('트랜잭션 영수증 동기화 대기');
          }
