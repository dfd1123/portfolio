...
module.exports = {
    ...
    try {
      ...
      if (includeFee) {
        ...
        if (calculatedAmount.isNegative()) {
          throw new Error('insufficient funds for transaction with fee');
        }
  ...
  async isConfirmCompleted(transactionHash, confirmNumber) {
    try {
      ...
      if (
        confirmTx === null
        || currentBlockNumber - confirmTx.blockNumber < confirmNumber
      ) {
        // 지정된 횟수의 컨펌이 완료되지 않음
        return false;
      }
  ...
  timeout(ms) {
    return new Promise((resolve, reject) => {
      const id = setTimeout(() => {
        clearTimeout(id);
        reject(new Error('Web3 certain request timeout'));
      }, ms);
    });
  },
