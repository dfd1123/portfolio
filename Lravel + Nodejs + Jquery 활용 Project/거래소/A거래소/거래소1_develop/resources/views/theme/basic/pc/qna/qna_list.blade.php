			...
			<div class="right_con">
				...
				<h1 class="cs_main_tit">                                
					...
					@if(Auth::user()->comunity_status == 1)
						<a href="#" id="qnaWrite" class="write_btn write_btn_st"> {{ __('support.do_one_to_one_ask') }}</a>
					@endif
				</h1>
...
<div class="modal_popup hide_modal" id="qnaWrite1">
	...
	<form method="post" enctype="multipart/form-data" action="{{route('qna_insert')}}" id="pna_write">
		...
		<div class="form-group">
			<input class="form-control" type="text" name="title" placeholder="{{ __('support.placeholder_title') }}">
		</div>
		
		<div class="form-group mb-3">
			<textarea class="form-control" name="description" placeholder="{{ __('support.placeholder_contents')}}">{!! __('support.textarea_contents') !!}</textarea>
		</div>

		<div class="form-group mb-4" style="height: 35px;">
			<input type="file" name="file1" id="thum_file" class="hide img_up" >
			<input type="text" value="" class="form-control mr-2 float-left filename_input" style="width: 75%;" readonly>
			<label class="form-control attach_btn float-left" for="thum_file" style="width: 23.2%;">{{ __('support.add_image') }}</label>
		</div>
