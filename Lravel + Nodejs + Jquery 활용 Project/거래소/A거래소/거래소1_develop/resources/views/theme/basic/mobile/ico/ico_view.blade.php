...
<!-- scrl_wrap -->
<div class="scrl_wrap m_ico_wrap m_ico_wrap-view">
        ...
        <div class="board_st_con ico_view_con">
            ...
            <div class="main_info">
                ...
                <div class="thumnail">
                    @if($ico->ico_thumnail == NULL)
                        <img src="{{asset('/storage/image/ico/no_image.jpg')}}" alt="{{$ico->ico_symbol}}" /> 
                    @else
                ...
                <section class="infos">
                    ...
                    <div class="ico_view_tit">
                        <span class="hd_coin">{{strtoupper($ico->ico_symbol)}}</span>
                        <h1 class="hd_tit">{{$ico->ico_title}}</h1>
                    </div>

                    <p class="info  pt-3 _intro text-center">
                        {{$ico->ico_intro}}
                    </p>
                    
                    <p class="info text-center _date">
                        {{ __('icoo.writer')}} <span class="idbywriten pl-2 pr-3 mr-3">{{$ico->w_name}}</span> {{ __('icoo.write_date')}} <span class="datewriten pl-2">{{$ico->created_at}}</span>
                    </p>

                    <p class="info _period">
                        <i class="img"></i>{{$ico->ico_from}} <br>
                        ~ {{$ico->ico_to}}
                    </p>
                    ...
                    <p class="info s_info _price">
                        ...
                        <span class="point_clr_ico">1 {{strtoupper($ico->ico_symbol)}} ≈ <span id="get_ico_coin_p">{{$ico->ico_coin_p}}</span> {{strtoupper($ico->ico_collect)}}</span>
                    </p>
                    ...
                    <form method="POST" action="{{route('ico_buy',$ico->id)}}" name="ico_buy" id="ico_buy_form">
                        @csrf
                        <input type="hidden" name="is_apply" value="true">
                    ...
                    <div class="info _totalsum">
                        ...
                        <label class="point_only_clr">{{ __('icoo.all_payment_price')}}</label>
                        ...
                        <ul class="buy_percent">
                            <li>
                                <button type="button" id="per_10">10%</button>
                            </li>
                            <li>
                                <button type="button" id="per_25">25%</button>
                            </li>
                            <li>
                                <button type="button" id="per_50">50%</button>
                            </li>
                            <li>
                                <button type="button" id="per_100">100%</button>
                            </li>
                        </ul>
                    ...
                    <p class="info s_info _buyamt">
                        ...
                        <span>
                            <b id="change_price" class="point_clr_ico_light pr-1">0</b>
                            <span class="currency_dark">{{strtoupper($ico->ico_symbol)}}</span>
                        </span>
                    </p>
                    ...
                    @auth
                        ...
                        @else
                            <button type="button" class="btn_style mt-3 @if($ico->ico_category != 1 || $ico->active != 1) not_active_btn @endif" onclick="func_buy()">{{ __('icoo.buy')}}</button> 
                        @endif
                    @else
                        <button type="button" class="btn_style mt-3 not_active_btn">{{ __('icoo.login')}}</button> 
                    @endauth
                    </form>
                </section>
            ...
            <section class="info_section info_section2">
                ...
                <table>
                    ...
                    <tr class="sns_tr">
                        ...
                        <td>
                            <a href="{{$ico->ico_sns4}}" target="_blank">INSTAGRAM</a>
            ...
            <button type="button" class="abslt_btn" onclick="location.href='{{ route('ico_list','all') }}'">{{ __('icoo.list')}}</button>
