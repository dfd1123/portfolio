...
<div class="m_myasset_wrap">
    ...
    <div class="bottom_wrap">
        ...
        <div class="con_1 con_box">
            ...
            <div class="scrl_wrap">
                ...
                <div class="status_bar">
                    ...
                    <div class="total_ast">
                        <p class="text-right">
                            <label class="float-left tit">{{ __('my_asset.my_krw') }}</label>
                            <span class="point_clr_3">{{ number_format($coin_balance_krw,0) }}</span>
                            <span class="currency">KRW</span>
                        </p>
                        <p class="text-right">
                            <label class="float-left tit">{{ __('my_asset.all_estimated_money') }}</label>
                            <span class="point_clr_3">{{ number_format($total_asset,0) }}</span>
                            <span class="currency">KRW</span>
                        </p>
                    </div>
                    ...
                    <ul class="my_ast_list bb-dddd">
                        <li class="bt-f0f0 pt-2">
                            ...
                            <span>{{ number_format($total_buying,0) }}</span><span class="currency">KRW</span>
                        </li>
                        <li class="bt-f0f0 pt-2">
                            <label>{{ __('my_asset.all_rating_revenue') }}</label>
                            <span>{{ number_format($total_eval_revenue,0) }}</span><span class="currency">KRW</span>
                        </li>
                        <li>
                            <label>{{ __('my_asset.all_rating_price') }}</label>
                            <span>{{ number_format($total_holding,0) }}</span><span class="currency">KRW</span>
                        </li>
                        <li>
				...
				@foreach($coins as $coin)
	                ...
	                <!-- 코인리스트박스 -->
	                <div class="my_coin_list_con {{ $result[$coin->api]['balance'] > 0 ? 'exist_balance' : '' }} {{ $coin->market == 'public'?'hide':'' }}" data-kind="{{$coin->market}}" >
	
	                    <div class="hd dp_table ">
	
	                        <div class="dp_table_cell _left added_arrow"  onclick="location.href='{{ route('marketKRW',$coin->api) }}'">
	                            <img src="{{ asset('/images/coin_img/'.$coin->api.'.png') }}" alt="coin_img" class="coin_symbol"/>
	                    ...
	                    <ul class="my_ast_list bb-dddd">
	                        <li class="bt-f0f0 pt-2">
                                ...
                                <span>{{ number_format($result[$coin->api]['avg'],0) }}</span><span class="currency">KRW</span>
	                        </li>
	                        <li class="bt-f0f0 pt-2">
	                            <label>{{ __('my_asset.buy_price') }}</label>
	                            <span>{{ number_format($result[$coin->api]['buying'],0) }}</span><span class="currency">KRW</span>
	                        </li>
	                        <li>
	                            <label>{{ __('my_asset.rating_price') }}</label>
	                            <span>{{ number_format($result[$coin->api]['eval'],0) }}</span><span class="currency">KRW</span>
	                        </li>
	                        <li>
        ...
        <div class="con_2 con_box">
            ...
            <div class="sch_option_wrap">
                ...
                <div class="option_group">
                    ...
                    <div class="inner_line">
                        <button class="btn_style btn_style_dark" onclick="search_date_history();">{{ __('my_asset.lookup') }}</button>
                    </div>
                </div>
			...
			<!-- 검색바 -->
            <div class="sch_bar bb_dark_line">
                <div class="coin_sch_bar">
                    <div class="inner">
                ...
                <div class="history_st">
                    ...
                    @forelse($historys as $history)
                    	...
                    	<li class="con self" name="{{ __('coin_name.'.$history->cointype) }}/{{ $history->cointype }}">
                            ...
                            <p class="info _coin coin_fn_style">
                                <!-- 마켓 유형 제외 <span>{{ strtoupper($history->currency) != 'USD' ? strtoupper($history->currency) : 'USDC' }}마켓</span> -->
                                <span>{!! __('coin_name.'.strtolower($history->cointype)) !!}(<u>{{ strtoupper($history->cointype) }}</u>)</span>
                            </p>
                            <p class="info _amt">
                                <label>{{ __('my_asset.quantity') }}</label>
					...
					@empty
						<li class="non_data">
                            <img src="/images/icon_notice.svg" alt="" class="btn_notice">{{ __('my_asset.property_sentence2') }}
						</li>
					@endforelse
                    </ul>
                </div>
                    ...
                    <ul class="list" id = "not_concluded_lists">
                    	@forelse($wait_trades as $wait_trade)
	                        ...
	                        <li class="con buy">
	                            ...
	                            <p class="info _coin">
	                            	<span class="point_market_clr">{{ strtoupper($wait_trade->currency) != 'USD' ? strtoupper($wait_trade->currency) : 'USDC' }}마켓</span>
						...
						@empty
							<li class="non_data">
                                <img src="/images/icon_notice.svg" alt="" class="btn_notice">{{ __('my_asset.property_sentence3') }}
							</li>
						@endforelse
                    </ul>
                </div>
