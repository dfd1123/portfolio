		...
		<div class="board_st_con">
			...
			<div class="right_con ">
                ...
                <div class="cs_table_view ios-scroll comunity_table_view">
                    <form method="POST" action="{{route('comunity.update', $board->id)}}?kind=board" id="board_form" enctype="multipart/form-data">
                        @csrf
                        <!--PUT 메소드로 사용가능-->
                        <input type="hidden" name="_method" value="PUT">
                        <input type="hidden" name="kind" value="board" />
                        <input type="hidden" name="board_name" value="{{$board_name}}" />
                        <input type="hidden" name="re_id" {{($board->re_id != NULL)?'value='.$board->re_id:''}} />
                        <input type="hidden" name="file1_ch" value="0">
                        <input type="hidden" name="file2_ch" value="0">
                        ...
                        <div class="date_input_divs">
                            ...
                            <div class="date_input_list">
                                ...
                                <span class="_subjt">
                                    <input type="text" class="_subjt_input" name="title" placeholder="{{ __('support.please_input_title') }}" value="{{$board->title}}" required="required">
                                </span>
                            </div>
                            <!-- // 글작성 제목 -->
                            ...
                            <div class="date_input_list">
                                ...
                                <span class="_subjt">
                                    <label class="check_secret_label">
                                        <input type="checkbox" id="check_secret" class="checkbox_style hide" {{($board->secret_key != NULL)?'checked=checked':''}}> 
                                        <label for="check_secret" class="checkbox_label"></label> {{ __('support.secret_board') }}
                                        @if(($board->secret_key != NULL))
                                        <input type="password" class="_pw_input" placeholder="{{ __('support.please_input_pw') }}" name="secret_key" value="" required>
                                        @endif
                                    </label>
                                </span>
                                ...
                                <span class="_subjt">
                                    <label>
                                    <input type="checkbox" name="notice" id="check_notice" class="checkbox_style hide" {{($board->notice == 1)?'checked=checked':''}}> 
                        ...
                        <div class="comunity_board_write_container mt-4">
                            <!-- TODO: 글작성할 수 있는 공간 -->
                            <textarea name="content" id="textarea" style="width:100%;min-height: 600px;border:none;resize:none; padding:10px;">{!! $board->content !!}</textarea>
                        </div>
                        ...
                        <div class="date_input_divs mt-4">
                            <div class="date_input_list" style="display:none;">
                                ...
                                <span class="_subjt filebox bs3-primary preview-image">
                                    <input class="upload-name" value="{{($file1 == '')?'파일선택':$file1}}" disabled="disabled" style="width: 300px;">
                            ...
                            <div class="date_input_list" style="display:none;">
                                ...
                                <span class="_subjt filebox bs3-primary preview-image">
                                    <input class="upload-name" value="{{($file2 == '')?'파일선택':$file2}}" disabled="disabled" style="width: 300px;">
...
<script>
    ...
    $('#file1').on('change', function(){
        $('input[name="file1_ch"]').val(1);
    });

    $('#file2').on('change', function(){
        $('input[name="file2_ch"]').val(1);
    });
