$( document ).ready(function() {
		...
		if(confirm('정말 해당 회원의 상태를 수정하시겠습니까?')){
			$.ajax({
				...
				success: function (data) { 
					if(data.status == 1){
						alert('해당 회원의 거래/입출금 사용을 허용하셨습니다.');
						$(this).val($.data(this, 'current'));
					}else if(data.status == 2){
						alert('해당 회원의 거래/입출금 사용을 정지하셨습니다.');
						$(this).val($.data(this, 'current'));
					}
				}
			});
	...
	$("select[name='comunity_status']").change(function(){
		var status = $(this).val();
		var id = $(this).data('id');
		
		if(confirm('정말 해당 회원의 상태를 수정하시겠습니까?')){
			$.ajax({
				url: '/admin/user_comunity_status_change',
				type: 'POST',
				/* send the csrf-token and the input to the controller */
				data: {_token: CSRF_TOKEN, status: status, id:id},
				dataType: 'JSON',
				...
				success: function (data) { 
					if(data.status == 1){
						alert('해당 회원의 커뮤니티 사용을 허용하셨습니다.');
						$(this).val($.data(this, 'current'));
					}else if(data.status == 0){
						alert('해당 회원의 커뮤니티 사용을 정지하셨습니다.');
						$(this).val($.data(this, 'current'));
					}
				}
			});
