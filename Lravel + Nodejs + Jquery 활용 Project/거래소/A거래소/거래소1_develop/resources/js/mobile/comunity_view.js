            ...
            data.comments.forEach(function (comment) {
                ...
                if (data.auth) {
                    ...
                    if (data.uid != comment.writer_id) {
                        if (!data.admin) {
                            templete.find('.reply_option_btn').remove();
                        } else {
                            templete.find('.reply_option_btn .option_btn.edit').remove();
                            templete.find('.reply_option_btn .option_btn.reple').remove();
                        }
                    }
                } else {
                    templete.find('.reply_option_btn').remove();
                ...
                comment.re_comments.forEach(function (re_comment) {
                    ...
                    if (data.auth ) {
                        ...
                        if (data.uid != comment.writer_id) {
                            if (!data.admin) {
                                templete2.find('.reply_option_btn').remove();
                            } else {
                                templete2.find('.reply_option_btn .option_btn.edit').remove();
                                templete2.find('.reply_option_btn .option_btn.reple').remove();
                            }
                        }
                    } else {
                        templete2.find('.reply_option_btn').remove();
            ...
            $.ajax({
                ...
                success: function (data) {
                    if(data.status){
                        ...
                        templete.find('.reply_option_btn').hide();
...
//댓글 수정 버튼 클릭시
$(document).on('click', '.mobile_comunity_wrap .option_btn.edit', function () {
    var content = $(this).parent().parent().parent().siblings('.contents').children('p').text();
...
//댓글 답글 버튼 클릭시
$(document).on('click', '.mobile_comunity_wrap .option_btn.reple', function () {
    ...
    $(this).parent().parent().parent().parent().siblings('.comment_write_wrap').append(templete);
    $(this).parent().parent().parent().parent().siblings('.comment_write_wrap').show();
    $(this).parent().hide();
});
//댓글 답글 버튼 클릭시
...
$(function () {
    ...
    $(document).on('click', '.reply_option_btn .option_btns_img', function () {

        if (click_num == 0) {

            $(this).siblings('.option_btn_list').show();
            click_num++;

        } else if (click_num == 1) {

            $(this).siblings('.option_btn_list').hide();
            click_num--;
