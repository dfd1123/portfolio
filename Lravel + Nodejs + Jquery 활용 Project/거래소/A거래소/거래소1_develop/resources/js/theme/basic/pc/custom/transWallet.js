...
/*입금주소-출금신청-입출금내역 탭메뉴*/
$('.transac_header ul li').click(function () {
	//누른 메뉴의 인덱스
	var index = $(this).data('index');

	//누른 메뉴 꾸밈효과 주고, 아닌 탭은 꾸밈효과 지운다
	$('.transac_header ul li[data-index=' + index + ']').addClass('active');
	$('.transac_header ul li[data-index!=' + index + ']').removeClass('active');

	//클릭한 순서값에 따라 매치되는 박스 보여주고 매치되지 않으면 숨긴다
	$('.transac_con_wrap .toggle_con[data-index=' + index + ']').show();
	$('.transac_con_wrap .toggle_con[data-index!=' + index + ']').hide();
});
/*//입금주소-출금신청-입출금내역 탭메뉴*/

$('.third_history_con .third_history_hd ul li').on('click', function () {
	//누른 입출금내역 필터
	var io = $(this).data('io');

	//누른 입출금내역 필터에 꾸밈효과 주고, 아닌 필터는 꾸밈효과 지운다
	$('.third_history_con .third_history_hd ul li[data-io=' + io + ']').addClass('active');
	$('.third_history_con .third_history_hd ul li[data-io!=' + io + ']').removeClass('active');
...
function select_coin(symbol) {
	...
	$.ajax({
		...
		$("#this_just_symbol").html(data.symbol);
		...
		if (symbol == 'USD') {
			...
			// 입금정보 삽입
			var first_deposit_info_ment = __.message.ments1.replace("*coinname*", data.coinname);
			$("#first_deposit_info_ment").html('' + first_deposit_info_ment + '');
			$("#first_deposit_info").html(__.message.my_deposit_addr.replace("*coinname*", data.coinname));
			if(symbol == 'ETH'){
			...
			if (data.api == 'eos' || data.api == 'xrp') {
				...
				$("#deposit_coin_address_qrcode").html('<img id="qrcode" src="https://chart.googleapis.com/chart?chs=200x200&amp;cht=qr&amp;chl=' + address_desti[0] + '&amp;choe=UTF-8" alt="qr">');
			...
			$('.pos_relative_box span').text(data.symbol_text);
			$('.withdraw_fee_box p span:nth-child(2) em').text(data.symbol_text);
			$('.total_price_box p span:nth-child(2) em').text(data.symbol_text);
		}
		...
		$('.third_history_con .third_history_hd ul li[data-io=all]').click();
	...
	if (symbol == 'USD') {
      ...
      return;
		...
		$.ajax({
			...
			type: "POST",
			data: { _token: CSRF_TOKEN, symbol: symbol, amt: amt, address: address, address_desti: address_desti },
			dataType: "JSON",
		}).done(function (data) {
	...
	$.ajax({
		...
		$.each(data.transaction_list, function () {
			if (this.cointype == 'USD' || this.cointype == 'usd') {
				...
				symbol_text = this.cointype.toUpperCase();
				button_text = '';
				...
				if (this.type == 'deposite') {
					str = '<tr class="in" data-kind="0" data-io="in">';
					str += '<td rowspan="2">' + __.message.deposit + '</td>';
				} else if (this.type == 'withdraw') {
					str = '<tr class="out" data-kind="1" data-io="out">';
				...
				if (this.type == 'deposite') {
					str += '<br>' + '(입금통장 표시 : ' + this.bankowner + ' ' + this.verification_code + ')';
				}
				str += '</p></td>';
				...
				if (this.type == 'withdraw' || this.type == 'out') {
					...
					str = '<tr class="out" data-kind="0" data-io="out">';
					str += '<td rowspan="2">' + __.message.withdraw + '</td>';
					str += '<td>';
					str += '<p>';
					...
					str += '</tr>';
					str += '<tr class="out" data-io="out">';
					str += '<td>';
					str += '<p>';
					str += '' + this.receiver_address + '';
					str += '<br>';
					if (this.type == 'withdraw' && this.status != 'withdraw_request') {
						str += '(<a href="https://etherscan.io/tx/' + this.tx_id + '" target="_blank">' + this.tx_id + '</a>)';
					}
					...
					str = '<tr class="in" data-kind="1" data-io="in">';
					str += '<td rowspan="2">' + __.message.deposit + '</td>';
					str += '<td>';
					str += '<p>';
					str += '' + this.total_amt + ' <span class="currency">' + symbol_text + '</span>';
	...
	if (amount < 1000) {
      ...
      return;
		...
		$.ajax({
			url: "/cash_deposite",
			type: "POST",
			data: { _token: CSRF_TOKEN, amount: amount, verification_code: verification_code },
			dataType: "JSON"
		}).done(function (data) {
			if (data.status == 'error1') {
				...
				swal({
					text: '입금 요청이 완료되었습니다. 화면에 보이는 스포홀딩스 계좌에 본인명의+인증코드로 입금해주세요. (예. ㅇㅇㅇ 123)',
					icon: "success",
					button: __.message.ok,
				}).then((confirm)=>{
					if(confirm){
...
function io_filter(io) {
	var target = $(".third_history_con .third_history_wrap .third_history_tbl tbody > tr[data-io=" + io + "]");

	if (io == 'all') {
		$(".third_history_con .third_history_wrap .third_history_tbl tbody > tr").show();
	} else {
		$(".third_history_con .third_history_wrap .third_history_tbl tbody > tr").hide();
		target.show();
	}
}