					...
					success : function(data) {
						if (data.data == 1) {
                            ...
                            if(data.event_info.status) {
                                ...
                                $('#n_event_modal_notify_m').html(data.event_info.number+'번째 이벤트에 당첨 되셨습니다.');
                                $('#n_event_modal_prize_m').html('');
                                $('#n_event_modal_prize_m').html('상품 : ' + data.event_info.productName);
                                $('.event_ul').append('<li>▶ '+data.event_info.number+'번째 행운 이벤트 당첨자 : '+data.event_info.nickName+'</li>');
                            }
