...
$( document ).ready(function() {
    ...
    $('.up_wrap.wait_wrap').click(function(e){  // 호가창 매도 리스트 클릭시 가격, 수량 데이터 연동
        ...
        $('#buy_fee').text($('#buy_coin_price').val());
        $('#sell_fee').text(0);
...
function buysell_coin_data(order_type, coin_currency,symbol){
	...
	if(ajax_run == 'n'){
		...
		if(order_type == "buy"){
					...
					success : function(data) {
						if (data.data == 1) {
                            ...
                            if(data.event_info.status) {
                                ...
                                $('#n_event_modal_notify').empty();
                                $('#n_event_modal_notify').html(data.event_info.number+'번째 이벤트에 당첨 되셨습니다.');
                                $('#n_event_modal_prize').empty('');
                                $('#n_event_modal_prize').html('상품 : ' + data.event_info.productName);
                            }
							...
							alertify.error('SPORTS COIN의 경우 20원 이하로 거래하실 수 없습니다.');
			...
			var balance_value = $('input[name="my_asset_coin"]').val().replace(/\,/g, ""); // 현재 잔액
        ...
        $.ajax({
            ...
            success : function(data) {
                var orders = data.map(function(item) {
                    return '<tr>' +
                        '<td style="width:12.5%;"><span>' + dateSet(item.local_time) + '</span></td>' +
                        '<td style="width:12.5%;"><span>' + item.coinname + '</span></td>' +
                        '<td style="width:9.5%;"><span>' + item.type_name + '</span></td>' +
                        '<td style="width:13.5%;"><span>' + item.ads_price + '</span></td>' +
                        '<td style="width:15.5%;"><span>' + item.ads_amount + '</span></td>' +
                        '<td style="width:10.5%;"><span>' + item.ads_percent + '%</span></td>' +
                        '<td style="width:13.5%;"><span>' + item.ads_total_amount + '</span></td>' +
                        '<td style="width:12.5%;">' +
        ...
        $.ajax({
            ...
            success : function(data) {
                ...
                if(data.length > 0){
                    $.each(data, function() {
                        str = '<tr>';
                        str += '<td style="width:13.6%;"><span>'+ dateSet(this.trade_date) +'</span></td>';
                        str += '<td style="width:13.6%;"><span>'+ this.coinname +'</span></td>';
                        str += '<td style="width:10.6%;"><span>'+ this.type_name +'</span></td>';
                        str += '<td style="width:20.6%;"><span>'+ this.trade_price +' '+ currency.toUpperCase() + '</span></td>';
                        str += '<td style="width:20.6%;"><span>'+ this.trade_amount +'</span></td>';
                        str += '<td style="width:20.6%;"><span>'+ this.trade_total_amount +' ' + currency.toUpperCase() + '</span></td>';
                        str += '</tr>';
...
function trade_cancel(id){
    ...
    var keydown = function(e) {
        if(e.keyCode == 13) {//enter
            $('.swal-button--yes').trigger('click');
        } else if(e.keyCode == 32) {//spacebar
            $('.swal-button--no').trigger('click');
        }
    };
        ...
        $.ajax({
            ...
            success : function(data) {
                ...
                $.each(data.result,function(index,item){
                    if(item.balance > 0){
                        $('[data-symbol="'+index.toUpperCase()+'"]').css("display","table-row");
                    }else{
                    ...
                    $('[data-symbol="'+index.toUpperCase()+'"]').find('[name=asset_revenue]').text(formatNumber(item.eval_revenue,0));
                });
            },
            complete: function() {
                isRefresh = false;
