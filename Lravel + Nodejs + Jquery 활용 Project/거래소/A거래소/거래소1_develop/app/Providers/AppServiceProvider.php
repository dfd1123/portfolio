    ...
    public function boot()
    {
        ...
        if (isset($_SERVER['HTTP_HOST']) && !empty($_SERVER['HTTP_HOST'])) {
            if ($_SERVER['HTTP_HOST'] == $_SERVER['SERVER_ADDR']) {
                $url = env('APP_URL')."/"; //Internals (cron, ...)
            } else {
                $protocol = 'https://';
        ...
        View::composer([ session('theme').'.pc.layouts.app', session('theme').'.mobile.layouts.app'], function ($view) {
            ...
            $company = DB::table('btc_settings')->where('id', session('market_type'))->first();
            ...
            if (Auth::check()) {
                if ($device == 'mobile') {
                    ...
                    foreach ($coins as $coin) {
                        ...
                        $coin_balance_price = bcmul($coin_balance, $coin->price_krw, 8); // 코인 보유 양 * 해당코인 가격 = 평가금액
