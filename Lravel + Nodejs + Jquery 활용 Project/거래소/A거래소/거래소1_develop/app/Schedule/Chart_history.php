...
class Chart_history
{
	public static function reset_coinmarketcap_price_all() {
		...
		$url = "https://api.coinmarketcap.com/v1/ticker/?convert=KRW"; // Where you want to post data
	...
	public static function update_market_info_new() {
		...
		foreach($coins as $coin){
			...
			DB::table('btc_coins')->where('symbol', strtoupper($coin->api))-> update([
	            '24h_volume_krw' => DB::raw("IFNULL( ( SELECT SUM(  `contract_coin_amt` )  24h_volume FROM `btc_trades_COIN_btc` WHERE `cointype` = '".$coin->api."' and currency = 'krw' and `created` >  UNIX_TIMESTAMP(NOW() - INTERVAL 24 HOUR) ),0)"),
	            'percent_change_24h_krw' => DB::raw("IFNULL(( price_krw - IFNULL( (SELECT `buy_coin_price` as opening_price FROM `btc_trades_COIN_btc` WHERE `cointype` = '".$coin->api."' and currency = 'krw' and `created` >  UNIX_TIMESTAMP( NOW() - INTERVAL 24 HOUR ) order by id asc limit 1 ),price_krw)  )* 100 / NULLIF((SELECT `buy_coin_price` as opening_price FROM `btc_trades_COIN_btc` WHERE `cointype` = '".$coin->api."' and currency = 'krw' and `created` >  UNIX_TIMESTAMP( NOW() - INTERVAL 24 HOUR ) order by id asc limit 1 ),0) , 0)"),
	            'price_change_24h_krw' => DB::raw("IFNULL( price_krw - IFNULL( (SELECT `buy_coin_price` as opening_price FROM `btc_trades_COIN_btc` WHERE `cointype` = '".$coin->api."' and currency = 'krw' and `created` >  UNIX_TIMESTAMP( NOW() - INTERVAL 24 HOUR ) order by id asc limit 1 ),price_krw)  , 0)"),
	            'price_all_24h_krw' => DB::raw("IFNULL( ( SELECT SUM(  `trade_total_sell` )  price_all_24h FROM `btc_trades_COIN_btc` WHERE `cointype` = '".$coin->api."' and currency = 'krw' and `created` >  UNIX_TIMESTAMP(NOW() - INTERVAL 24 HOUR) ),0)"),
	            'opening_price_krw' => DB::raw("IFNULL(( SELECT `buy_coin_price` as opening_price FROM `btc_trades_COIN_btc` WHERE `cointype` = '".$coin->api."' and currency = 'krw' and `created` >  UNIX_TIMESTAMP( NOW() - INTERVAL 24 HOUR ) order by id asc limit 1 ),0)"),
	        	'closing_price_krw' => DB::raw("IFNULL(( SELECT `buy_coin_price` as closing_price FROM `btc_trades_COIN_btc` WHERE `cointype` = '".$coin->api."' and currency = 'krw' and `created` >  UNIX_TIMESTAMP( NOW() - INTERVAL 24 HOUR ) order by id desc limit 1 ),0)"),
	            'min_price_krw' => DB::raw("IFNULL( (select min(`buy_coin_price`) min_price from `btc_trades_COIN_btc` where `cointype` = '".$coin->api."' and currency = 'krw' and `created` >  UNIX_TIMESTAMP(NOW() - INTERVAL 24 HOUR))  , 0)"),
	            'max_price_krw' => DB::raw("IFNULL( (select max(`buy_coin_price`) max_price  from `btc_trades_COIN_btc` where `cointype` = '".$coin->api."' and currency = 'krw' and `created` >  UNIX_TIMESTAMP(NOW() - INTERVAL 24 HOUR))  , 0)"),
	            'updated_dt' => DB::raw('now()'),
	            'updated' => time(),
	        ]);
		}
