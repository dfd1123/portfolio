        ...
        foreach($p2ps as $p2p){
            ...
            if($auth){
                if(Auth::user()->status == 2){
                    $str .= '<button class="not_active_btn stop_user_id_warning" type="button">계정 정지</button> ';
                }else if($p2p->state == 'on'){
                    $str .= '<button class="apply_btn write_btn p2pApply" data-id="'.$p2p->id.'" type="button">'.__('p2p.'.$p2p->type).__('ptop.apply1').'</button>';
                }else if($p2p->state == 'onProgress'){
                    $str .= '<button class="not_active_btn" type="button">'.__('ptop.ing').'</button> ';
                }elseif($p2p->state == 'complete'){
                    $str .= ' <button class="not_active_btn" type="button">'.__('ptop.complete').'</button> ';
                }elseif($p2p->state == 'stop'){
                    $str .= '<button class="not_active_btn" type="button">'.__('ptop.stop').'</button> ';
                }
            }else{
                if($p2p->state == 'on'){
                    $str .= '<button class="apply_btn write_btn" type="button" onclick="location.href=\''.route("login").'\'">'.__('p2p.'.$p2p->type).__('ptop.apply1').'</button>';
                }else if($p2p->state == 'onProgress'){
                    $str .= '<button class="not_active_btn" type="button">'.__('ptop.ing').'</button> ';
