...
class MyAssetAjaxController extends Controller
{
    ...
    public function show_more_history(Request $request){
		...
		->where(function ($query) use ($uid){
			$query->where('buyer_uid',$uid)->orwhere('seller_uid',$uid);
		})->whereBetween('created_dt',[$start_date, $end_date])->select('btc_trades_COIN_btc.*','btc_coins.decimal_krw','btc_coins.decimal_btc','btc_coins.decimal_eth','btc_coins.decimal_krw')->orderBy('id','desc')->offset($offset)->limit(10)->get();
		...
		foreach($historys as $history){
			...
			if($history->buyer_uid == $history->seller_uid){
				...
				$result[$i]['calcul_price'] = number_format(bcsub($history->trade_total_sell,bcmul(bcmul($history->trade_total_sell,$setting->sell_comission,$history->{'decimal_'.strtolower($history->currency)}),"0.01",$history->{'decimal_'.strtolower($history->currency)}),$history->{'decimal_'.strtolower($history->currency)}), $history->{'decimal_'.strtolower($history->currency)});
