...
class TranswalletAjaxController extends Controller
{
    ...
    public function select_coin(Request $request){
		...
		if($symbol != "KRW" && $symbol != "USD"){
            ...
            $Settings = Settings::Settings();
            if($security_lv == 0){
	            $withdraw_limit_amt = 0;
	        }else if($security_lv == 1){
	            $withdraw_limit_amt = $Settings->lv1_max;
	        }else if($security_lv == 2 ){
	            $withdraw_limit_amt = $Settings->lv2_max;
	        }else if($security_lv == 3 ){
	            $withdraw_limit_amt = $Settings->lv3_max;
	        }else if($security_lv == 4){
	            $withdraw_limit_amt = $Settings->lv4_max;
	        }else if($security_lv == 5){
	            $withdraw_limit_amt = $Settings->lv5_max;
	        }
		}
    ...
    public function send_transaction(Request $request){
        ...
        $amt = $request->amt;
        $address = $request->address;
        $address_desti = $request->address_desti;
        $uid = Auth::user()->id;
        $username = Auth::user()->username;
        ...
        //주소가 내부인지 외부인지 구별
        if(strtolower($symbol) == 'xrp' || strtolower($symbol) == 'eos'){
            $internal_external = DB::table('btc_users_addresses')->where('address_'.strtolower($symbol),$address)->where('address_desti_'.strtolower($symbol),$address_desti)->first();
        }else{
        ...
        if($internal_external == null){ // null 이면 외부로 출금 아니면 내부로 출금
            DB::table('btc_coin_send_request')->insert([ //출금자 출금 요청정보 삽입
                ...
                'receiver_address_desti' => $address_desti,
                'req_amount' => $amt,
                'send_fee' => $send_fee,
            ...
            if(bcmul($amt , $coin->price_krw, 8) > 1000000){ // 100만원 이상일경우 관리자 승인 받고 안받고
                ...
                $withdraw_push = DB::table('users')->where('id',$uid)->select('push_token')->first();
    ...
    public function refresh_withdraw_history(Request $request){
		...
		if($symbol == 'KRW'){
			...
			)->where('uid',Auth::id())->orderBy('id','desc');
		}else{
    ...
    {
        $Settings = Settings::Settings();
        $url = $Settings->url_io."api/update_eth_erc_balance.php";
        $postdata = [
	...
	public function cash_deposite(Request $request){
		if(My_info::krw_requested_or_not()){
            ...
            $verification_code = $request->verification_code; //인증코드
	...
	public function cash_withdraw(Request $request){
		if(My_info::krw_requested_or_not()){
			...
			$coin = DB::table('btc_coins')->where('symbol',$symbol)->first(); //수수료 가져옴
			$user_info = DB::table('users')->where('id',Auth::user()->id)->first(); // 유저정보
			$user_addresses = DB::table('btc_users_addresses')->where('uid',Auth::user()->id)->first(); //krw 잔액 체크
			$user_security_info = DB::table('btc_security_lv')->where('uid',Auth::user()->id)->first(); //보안레벨의 은행정보 확인
			
			if(bcadd($amount,$coin->send_fee,0) > bcadd($user_addresses->available_balance_krw + $user_addresses->pending_received_balance_krw,0) || $amount <= 0){
				$response = array(
		            "status" => "error1",
		        );
	...
	public function cash_cancel(Request $request){
		...
		if($type == 0){
            ...
            try{
                DB::beginTransaction();
                $this_cash_info = DB::table('btc_krw_io')->where('id',$id)->sharedLock()->where('status','withdraw_request')->first();
