...
class CustomController extends Controller
{
    ...
    public function newsplash(Request $request){
        $offset = $request->offset;
        $str = '';

        $newsplashs = DB::connection('mysql_sub')->table('btc_newsplash_'.config('app.country'))->orderBy('created','desc')->offset($offset)->limit(10)->get();

        foreach($newsplashs as $newsplash){
            $str .= '<tr><td class="list_tit">';
            $str .= '<a href="/newsplash/'.$newsplash->id.'"><span>'.$newsplash->title.'</span></a>';
            $str .= '</td>';
            $str .= '<td class="date_ymd">'.date("Y-m-d", $newsplash->created).'</td></tr>';
            $offset++;
        }
