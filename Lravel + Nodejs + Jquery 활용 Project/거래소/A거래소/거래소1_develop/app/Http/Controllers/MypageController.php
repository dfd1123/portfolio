...
class MypageController extends Controller
{
    ...
    public function otp_setting(Request $request){
        ...
        if($status == 3) {
            /* sudo apt-get install php7.2-gd 필요 */
            ...
            $status = DB::table('btc_security_lv')->where('uid', Auth::user()->id)->update(['google_pin' => $google2fa_secret]);

            if($status){
                AirDrop::drop(Auth::user()->id, 'security_otp');
            }
        }
    ...
    public function security_setting(Request $request){
        ...
        $status = Secure::secure_long_verified();
        /*
        if($status == 4){
            return Redirect::route('main');
        }
        */
        ...
        $views->status = Secure::secure_long_verified();
	...
	public function checkplus_success(Request $request){
		...
		if(is_array($nice_info)){
            ...
            $birthdate = $nice_info['birthdate'];
            $gender = $nice_info['gender'];
			$status = 1;
    ...
    public function member_out(){
        ...
        $views = view(session('theme').'.'.$this->device.'.'.'mypage.member_out');
        $views->status = Secure::secure_long_verified();
        return $views;
    }

    public function member_secession(Request $request){
        if(!Auth::check()){
            return Redirect::route('main');
        }
        ...
        Auth::logout();
        return redirect('/home')->with('jsAlert', '탈퇴하셨습니다. 이제 해당 아이디로는 로그인을 할 수 없습니다.');
    }

    public function event_coupon(){
        if(!Auth::check()){
            return Redirect::route('main');
        }
        $event_coupon = DB::table('event_coupon')->where('email',Auth::user()->email)->first();
        
        $views = view(session('theme').'.'.$this->device.'.'.'mypage.event_coupon');
        $views->status = Secure::secure_long_verified();
        $views->event_coupon = $event_coupon;
        return $views;
    }
}
