...
class RegisterController extends Controller
{
    ...
    */
    ...
    {
        $this->middleware('guest')->except('register_complete');
		$agent = new Agent();
    ...
    protected function create(array $data)
    {
        ...
        if($data['nickname_cv']==NULL || $data['nickname_cv']==0){
            return redirect()->route('register')->with('jsAlert','닉네임 중복검사를 하지 않으셔서 처음으로 돌아갑니다.');
        }

        $check_mobile = DB::table('users')->where('mobile_number', str_replace('-','',$data['mobile_number']))->first();
        if($check_mobile != null){
            return redirect()->route('register')->with('jsAlert','가입중이거나 탈퇴 처리된 회원의 휴대폰 번호를 사용하셔서 처음으로 돌아갑니다.');
        }
        ...
        if($data['register_type'] == 1){
            ...
            $user =  User::create([
                ...
                'company_detail_address' => $data['company_detail_address'],
                'comunity_count' => 10,
                'comment_count' => 20,
            ]);
        }
        ...
        if($setting->recommender_yn == 1){
            ...
            $ref_user = DB::table('users')->where('email',$refid)->first();
            if(!empty($ref_user)){
                DB::table('btc_users_addresses')->where('label', $ref_user->username)->increment('available_balance_krw',$setting->recommender_point);
            }
        }
