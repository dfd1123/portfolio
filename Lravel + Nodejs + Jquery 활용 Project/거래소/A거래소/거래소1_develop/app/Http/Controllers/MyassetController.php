...
class MyassetController extends Controller
{
    ...
    public function index()
    {
		...
		foreach($coins as $coin){
            ...
            //okj 평균단가 초기화 2019-12-09
			$avg_id = My_info::avg_trades_id($coin->symbol);

			$coin_buy_amt = My_info::buy_amt_new($coin->symbol,$avg_id); //코인별 총매수량
			$coin_buy_total = My_info::buy_total_new($coin->symbol,$avg_id); //코인별 총 (가격 * 양)
            /*
            ...
            */
			...
			$coin_balance_price = bcmul($coin_balance, $coin->price_krw,0); // 코인 보유 양 * 해당코인 가격 = 보유평가금액
