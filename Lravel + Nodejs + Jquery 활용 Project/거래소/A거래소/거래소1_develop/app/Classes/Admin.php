...
class Admin {
    ...
    public function cancel_coin_io($request_id,$status){ 
        $result = false;
        $coin_io = DB::table('btc_coin_send_request')->where('id',$request_id)->where('status','withdraw_request')->first();
        if (isset($coin_io->sender_userid)) {
    ...
    public function manual_confirm($tx_id, $id) { 
        $manual = DB::table('btc_coin_send_request')->where('id', $id)->where('status','withdraw_request')->first();
    ...
    public function alarm_withdraw_confirm($username, $cointype, $receiver_address, $req_amount) {
        ...
        if(strtolower($cointype) == 'krw'){
            if ($alarm_io_sms == 1) {
                $message = trans('alarm.alarm_cash_sms_withdraw', ['cointype' => $cointype, 'address' => $receiver_address, 'amount' => $req_amount], $country);
