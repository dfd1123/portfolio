...
class Trade_new {
    ...
    function Market_buy_execute_trade( $ads_id ){
        ...
        DB::select(
            'call buy_trades(?,?,?)',
            array($ads_id,$market->buy_comission, $market->sell_comission )
        );
        $btc_ads = DB::table('btc_ads_btc')->where('id', $ads_id)->first();
        
        $history = DB::table('btc_trades_COIN_btc')->where('currency', $btc_ads->currency)->where('cointype', $btc_ads->cointype)->orderBy('id', 'DESC')->first();
        if ($btc_ads->buy_COIN_amt != $btc_ads->buy_COIN_amt_total) {
            $this->update_market_price($history->buy_coin_price, $history->contract_coin_amt, $history->currency, $history->cointype);
            //푸시날리기
            $user_push = DB::table('users')->where('id',$history->seller_uid)->select('push_token')->first();
            $tokens = array();
            $tokens[] = $user_push->push_token;
            
            $data = array();
            $data['title'] = "체결 사항";
            $data['body'] = strtoupper($history->cointype)." 판매체결이 이루어졌습니다.";
            $fcm_return_key = FcmPush::push_one($tokens, $data);

            $response_buy = array(
                "uid" => $btc_ads->uid,
                "type" => 'buy',
                "cointype" => strtoupper($btc_ads->cointype),
                "amount" => $btc_ads->buy_COIN_amt_finished,
            );
        }
        ...
        try {
            DB::beginTransaction();
            $btc_ads = DB::table('btc_ads_btc')->where('id', $ads_id)->sharedLock()->first();
            ...
            if($btc_ads != null){
                ...
                ->orderByRaw('sell_coin_price ASC, created ASC')->sharedLock()->get();
                ...
                if ($buy_COIN_amt == $sell_COIN_amt) { // 매도량과 매수량이 같을 때
                ...
                } elseif ($buy_COIN_amt > $sell_COIN_amt) { // 매수량이 클때
                ...
                } elseif ($buy_COIN_amt < $sell_COIN_amt) { // 매도량이 클때
                    ...
                    $buyer_info = DB::table('btc_users_addresses')->where('uid', $buyer_uid)->first();
                    $seller_info = DB::table('btc_users_addresses')->where('uid', $seller_uid)->first();

                    $pending_history_id = DB::table('pending_history')->insertGetId([
                        'buy_ad_id' => $buy_ad_id,
                        'buy_available_before' => $buyer_info->available_balance_krw,
                        'buy_available_after' => bcsub($buyer_info->available_balance_krw, $trade_pay_amt_buy_with_fee_original,17),
                        'buy_pending_before' => $buyer_info->pending_received_balance_krw,
                        'buy_pending_after' => bcadd($buyer_info->pending_received_balance_krw, $trade_pay_amt_buy_with_fee, 17),
                        'buy_price' => $trade_pay_amt_buy_original,
                        'buy_fee' => $buy_trade_fee_original,
                        'buy_price_total' => $trade_pay_amt_buy_with_fee_original,
                        'buy_cointype' => 'KRW',
                        'sell_ad_id' => $sell_ad_id,
                        'seller_available_before' => $seller_info->available_balance_krw,
                        'seller_available_after' => bcadd($seller_info->available_balance_krw, $trade_pay_amt_sell_with_fee,17),
                        'sell_price' => $trade_pay_amt_sell,
                        'sell_fee' => $sell_trade_fee,
                        'sell_price_total' => $trade_pay_amt_sell_with_fee,
                        'sell_cointype' => $trade_coin_symbol,
                    ...
                    $this->write_usd_io_history(5269, "margin", "confirmed", $buy_trade_fee, "ads", $buy_ad_id, "구매수수료", $process_currency);
                    
                    $log_text = $log_text." (6). 판매수수료 ".$sell_trade_fee." 를 admin user에게 준다. <br>\n";
                    $this->write_usd_io_history(5269, "margin", "confirmed", $sell_trade_fee, "ads", $sell_ad_id, "판매수수료", $process_currency);
    ...
    function Market_sell_execute_trade( $ads_id ){ // 판매건 단건에 대한 매칭을 처리한다.	
        ...
        $history = DB::table('btc_trades_COIN_btc')->where('currency', $btc_ads->currency)->where('cointype', $btc_ads->cointype)->orderBy('id', 'DESC')->first();
        if ($btc_ads->sell_COIN_amt != $btc_ads->sell_COIN_amt_total) {
            $this->update_market_price($history->buy_coin_price, $history->contract_coin_amt, $history->currency, $history->cointype);
            //푸시날리기

            $user_push = DB::table('users')->where('id',$history->buyer_uid)->select('push_token')->first();
            $tokens = array();
            $tokens[] = $user_push->push_token;
            
            $data = array();
            $data['title'] = "체결 사항";
            $data['body'] = strtoupper($history->cointype)." 구매체결이 이루어졌습니다.";
            $fcm_return_key = FcmPush::push_one($tokens, $data);
            ...
            $response_sell = array(
                ...
                "type" => 'sell',
                "cointype" => strtoupper($btc_ads->cointype),
                "amount" => $btc_ads->sell_COIN_amt_finished,
            );
        }
        ...
        try {
            ...
            if($btc_ads != null){
                ...
                ->orderByRaw('buy_coin_price DESC, created ASC')->sharedLock()->get();
                ...
                foreach($btc_ads2 as $btc_ad2){
                    ...
                    $log_text = $log_text." ** . 수수료 차감 판매 금액 : ".$trade_pay_amt_buy.". <br>\n";
                    $log_text = $log_text." ** .수수료 포함 구매금액	 : ".$trade_pay_amt_buy_with_fee.". <br>\n";
                    $log_text = $log_text." ** .매수자 기준 거래 금액	 : ".$trade_pay_amt_buy_with_fee_minus.". <br>\n";
                    ...
                    $pending_history_id = DB::table('pending_history')->insertGetId([
                        'buy_ad_id' => $buy_ad_id,
                        'buy_available_before' => $buyer_info->available_balance_krw,
                        'buy_available_after' => bcsub($buyer_info->available_balance_krw, $trade_pay_amt_buy_with_fee,17),
                        'buy_pending_before' => $buyer_info->pending_received_balance_krw,
                        'buy_pending_after' => bcadd($buyer_info->pending_received_balance_krw, $trade_pay_amt_buy_with_fee, 17),
                        'buy_price' => $trade_pay_amt_buy,
                        'buy_fee' => $buy_trade_fee,
                        'buy_price_total' => $trade_pay_amt_buy_with_fee,
                        'buy_cointype' => 'KRW',
                        'sell_ad_id' => $sell_ad_id,
                        'seller_available_before' => $seller_info->available_balance_krw,
                        'seller_available_after' => bcadd($seller_info->available_balance_krw, $trade_pay_amt_sell_with_fee,17),
    ...
    function write_usd_io_history($uid, $type, $status, $amount, $rel_type, $rel_id, $memo,$process_currency){
        ...
        if($userinfo != null) {
                ...
                $balance_after = bcadd($balance_before , $amount,8);
    ...
    function update_market_price($price, $amount, $currency, $cointype){
        ...
        $price_change = bcsub($price, $coin->{'opening_price_'.strtolower($currency)}, 8);
        if($coin->{'opening_price_'.strtolower($currency)} == 0){
            $price_change_percent = 0;
        }else{
            $price_change_percent = bcmul(bcdiv($price_change,$coin->{'opening_price_'.strtolower($currency)},8),100,2);
        }
        ...
        DB::table('btc_coins')->where('symbol', strtoupper($cointype))-> update([
            ...
            'percent_change_24h_'.strtolower($currency) => $price_change_percent,
