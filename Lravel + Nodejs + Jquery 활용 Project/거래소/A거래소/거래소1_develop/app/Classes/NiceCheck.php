...
class NiceCheck {
    public function NiceCheck_main()
    {
    	...
    	$sitecode = "BP785";				// NICE로부터 부여받은 사이트 코드
    	$sitepasswd = "NY1ypA3A7tCt";			// NICE로부터 부여받은 사이트 패스워드
	...
	public function NiceCheck_success()
    {
	    ...
	    if ($enc_data != "") {
	        ...
	        if ($plaindata == -1){
				...
				$name_utf8 = $this->GetValue($plaindata , "UTF8_NAME"); //charset utf8 사용시 주석 해제 후 사용
				$fullname = URLDecode($name_utf8);
				...
				$gender = $this->GetValue($plaindata , "GENDER"); // 남자:1, 여자:0
				$adultdate = date("Ymd",strtotime("-19 years"));
				...
				if((int)$birthdate > (int)$adultdate){
					return "미성년자는 사용 불가능합니다.";
					$requestnumber = "";
	                $responsenumber = "";
	                $authtype = "";
	                $name = "";
				...
				return ["mobileno" => $mobileno, "name" => $name, "birthdate" => $birthdate, "gender" => $gender, "returnMsg" => "성공"];
				//return ["name" => $fullname, "birthdate" => $birthdate, "gender" => $gender, "returnMsg" => "성공"];
	        }
	    }
	}
