...
class Trade {
    ...
    function Market_buy_execute_trade( $ads_id ){
        ...
        $btc_ads = DB::table('btc_ads')->where('id', $ads_id)->first();
        ...
        if($btc_ads != null){
            $log_text = $log_text."\n<br>";
            $log_text = $log_text."매수건 거래번호 : ".$btc_ads->id."\n<br>";
            $log_text = $log_text." [ 매수잔량 : ".$btc_ads->buy_COIN_amt." ]"."\n<br>";

            $trade_coin_symbol = $btc_ads->cointype; // 코인종류
            $buy_remain_amt = $btc_ads->buy_COIN_amt; // 구매잔량
            
            $btc_ads2 = DB::table('btc_ads')->where('type','sell')->where('status','OnProgress')->where('cointype',$trade_coin_symbol)
            ...
            foreach($btc_ads2 as $btc_ad2){
                ...
                //$trade_pay_amt_sell = $this->convertFloat( ( $contract_coin_amt * 100 )  * ( $sell_coin_price * 100 ) / 10000 ) ; // 매도자 기준 거래 금액 
				//$trade_pay_amt_buy = $this->convertFloat( ( $contract_coin_amt * 100 ) * ( $buy_coin_price * 100 ) / 10000 ) ; // 매수자 기준 거래 금액 
                ...
                if($process_currency == 'KRW') {
                    //아직 작성안함
                } else if($process_currency == 'BTC') {
                    //아직 작성안함
                } else if($process_currency == 'USD') {											
                    ...
                    $log_text = $log_text." (1). 매수자에게서  ".$trade_pay_amt_buy_with_fee_minus." 만큼 거래금액으로 차감하고 <br>\n";
                        $this->write_usd_io_history($buyer_uid, "buy_COIN", "confirmed", $trade_pay_amt_buy_with_fee_minus , "ads" , $buy_ad_id, "코인 매수" );

                    $log_text = $log_text." (2). 매수자의 pending 금액에  ".$trade_pay_amt_buy_with_fee." 만큼 더해준다. <br>\n";
                        $this->remove_pending_usd($buyer_uid, $trade_pay_amt_buy_with_fee_original, 'coin buy-1-'.$buy_ad_id );
                    ...
                    $log_text = $log_text." (4). 매도자에게  판매금액에서 수수료를 차감한 금액인 ".$trade_pay_amt_sell_with_fee." 만큼 준다. <br>\n";
                        $this->write_usd_io_history($seller_uid, "sell_COIN", "confirmed", $trade_pay_amt_sell_with_fee , "ads" , $sell_ad_id, "코인 매도" );
                        
                    $log_text = $log_text." (5). 구매수수료 ".$buy_trade_fee." 를 admin user에게 준다. <br>\n";
                        $this->write_usd_io_history('sbtr01', "margin", "confirmed", $buy_trade_fee , "ads" , $buy_ad_id, "구매수수료" );
                        
                    $log_text = $log_text." (6). 판매수수료 ".$sell_trade_fee." 를 admin user에게 준다. <br>\n";
                        $this->write_usd_io_history('sbtr01', "margin", "confirmed", $sell_trade_fee , "ads" , $sell_ad_id, "판매수수료" );
    ...
    function Market_sell_execute_trade( $ads_id ){ // 판매건 단건에 대한 매칭을 처리한다.	
        ...
        if($btc_ads != null){
            ...
            $btc_ads2 = DB::table('btc_ads')->where('type','buy')->where('status','OnProgress')->where('cointype',$trade_coin_symbol)
            ...
            foreach($btc_ads2 as $btc_ad2){
                ...
                //$trade_pay_amt_sell = convertFloat( ( $contract_coin_amt * 100 )  * ( $sell_coin_price * 100 ) / 10000 ) ; // 매도자 기준 거래 금액 
                //$trade_pay_amt_buy = convertFloat( ( $contract_coin_amt * 100 ) * ( $buy_coin_price * 100 ) / 10000 ) ; // 매수자 기준 거래 금액 
                ...
                if($process_currency == 'KRW') {
                    ...
                    $log_text = $log_text." (2). 매수자의 pending 금액에  ".$trade_pay_amt_buy_with_fee." 만큼 더해준다. <br>\n";
                        $this->remove_pending_usd($buyer_uid, $trade_pay_amt_buy_with_fee, 'coin buy-2-'.$buy_ad_id );
    ...
    function update_ad_sell($sell_ad_id, $sell_COIN_amt_finished, $sell_COIN_amt_remain){ //매도 주문을 업데이트
        ...
        $ads = DB::table('btc_ads')->where('id', $sell_ad_id)->first();
        ...
        DB::table('btc_ads')->where('id', $sell_ad_id)->update([
    ...
    function update_ad_buy($buy_ad_id, $buy_COIN_amt_finished, $buy_COIN_amt_remain){
        ...
        $ads = DB::table('btc_ads')->where('id', $buy_ad_id)->first();
        ...
        DB::table('btc_ads')->where('id', $buy_ad_id)->update([
    ...
    function write_trade_history($buyer_uid, $seller_uid, $buyer_username, $seller_username, $buy_ad_id, $sell_ad_id,  $contract_coin_amt, $buy_coin_price, $sell_coin_price,$trade_price_margin,  $trade_Cash_amt_buy, $trade_Cash_amt_sell, $trade_COIN_amt_buy, $trade_COIN_amt_sell, $trade_margin, $tx_id, $krw_io_id, $pay_tx_id ,$process_currency,$cointype, $last_trade_kind){
        ...
        DB::table('btc_trades_COIN')->insert([
            'buyer_uid' => $buyer_uid,
            ...
            'trade_'.strtolower($process_currency).'_amt_buy' => $trade_Cash_amt_buy,
            'trade_'.strtolower($process_currency).'_amt_sell' => $trade_Cash_amt_sell,
            'trade_COIN_amt_buy' => $trade_COIN_amt_buy,
            'trade_COIN_amt_sell' => $trade_COIN_amt_sell,
    ...
    function write_usd_io_history($uid, $type, $status, $amount, $rel_type, $rel_id, $memo){
        $username = "";
        $balance_before = 0;
        ...
        if($userinfo != null) {
                ...
                $balance_before = $userinfo->available_balance_usd;
                ...
                // 사용자 테이블 업데이트
                DB::table('btc_users_addresses')->where('uid',$uid)->increment('available_balance_usd', $amount);
    
        } // if userinfo
    }

    function remove_pending_usd($buyer_uid, $trade_usd_amt_buy, $reason = 'trade'){
        
        /*
        DB::table('btc_pending_history')->insert([
            'cointype' => 'usd',
            'uid' => $buyer_uid,
            'pending_value' => $trade_usd_amt_buy,
            'reason' => 'trade',
            'created' => DB::raw('now()'),
        ]);
        */        

        DB::table('btc_users_addresses')->where('uid', $buyer_uid)->increment('pending_received_balance_usd', $trade_usd_amt_buy);
        
    }

    function remove_pending_krw($buyer_uid, $trade_krw_amt_buy){
        DB::table('btc_users_addresses')->where('uid', $buyer_uid)->increment('pending_received_balance_krw', $trade_krw_amt_buy);
    }
    
    
    function remove_pending_btc($buyer_uid, $trade_btc_amt_buy){
        DB::table('btc_users_addresses')->where('uid', $buyer_uid)->increment('pending_received_balance_btc', $trade_krw_amt_buy);
    }
    ...
    function remove_pending_COIN($seller_uid, $trade_COIN_amt_sell, $cointype, $reason = 'trade'){
        ...
        DB::table('btc_pending_history')->insert([
            'cointype' => $cointype,
            'uid' => $seller_uid,
            'pending_value' => $trade_COIN_amt_sell,
            'reason' => $reason,
            'created' => DB::raw('now()'),
        ]);
        */
    ...
    function update_market_price($price, $amount, $currency, $cointype){
        ...
        DB::table('btc_prices')->where('currency', strtoupper($cointype).strtoupper($currency))->where('source', 'trade')
        ->update([
            'price' => $price,
            'created' => DB::raw('now()'),
            'time' => $time,
		...
		if($coin->max_price > $price){
			$max_price = $coin->max_price;	
		}else{
			$max_price = $price;
		}
		
		if($coin->min_price < $price){
    ...
    function update_market_price_minor($price, $currency, $cointype){
        $time = time();
    ...
    // ---------------------------- 구매 취소 ---------------------------
        ...
        $btc_adss = DB::table('btc_ads')->where('type','buy')->where('status','CancelRequest')
                    ->where('currency',$process_currency)->where('buy_COIN_amt','>',0)
        ...
        foreach($btc_adss as $btc_ads){
            ...
            $pending_btc_amt = 0;
            $pending_usd_amt = 0;
            $pending_btc_amt_with_fee = 0;
            $pending_krw_amt_with_fee = 0;
            $pending_usd_amt_with_fee = 0;

            if($process_currency == 'KRW') {
                $pending_krw_amt = $this->convertFloat( $buy_COIN_amt * $buy_coin_price );
                $pending_krw_amt_with_fee =  $pending_krw_amt + round( $pending_krw_amt * $market->buy_comission * 0.01 ,8 ) ;
                $pending_btc_amt = 0;
            } else if($process_currency == 'USD') {
                ...
                $pending_krw_amt = 0;
                $pending_btc_amt = $this->convertFloat( $buy_COIN_amt * $buy_coin_price );	
                $pending_btc_amt_with_fee =  $pending_btc_amt + round( $pending_btc_amt * $market->buy_comission * 0.01 ,8 ) ;	
            }

            $buyer_uid = $btc_ads->uid;

			$log_text = $log_text."\n\n";
			$log_text = $log_text."구매취소 IDX: ".$btc_ads->id."\n";
			$log_text = $log_text."구매취소 유저: ".$btc_ads->userid."\n";

			if($process_currency == 'USD') {
				$log_text = $log_text."팬딩금액 반환: ".(float)$pending_usd_amt_with_fee.$btc_ads->currency."\n";
			} else if($process_currency == 'KRW') {
				$log_text = $log_text."팬딩금액 반환: ".$pending_krw_amt_with_fee.$btc_ads->currency."\n";
			} else if($process_currency == 'BTC') {			
				$log_text = $log_text."팬딩금액 반환: ".$pending_btc_amt_with_fee.$btc_ads->currency."\n";
            }

            // 요청건 상태값 변경
            
            DB::table('btc_ads')->where('id',$btc_ads->id)->update([
                "status" => 'Cancel',
            ]);
				
			info('취소하는 구매 '.$btc_ads->currency.' 금액 : '.$pending_usd_amt_with_fee);	
				
			if($btc_ads->currency == 'KRW') {
				$this->remove_pending_krw($buyer_uid, $pending_krw_amt_with_fee); // buy_COIN_amt * buy_coin_price 만큼 uid 유저의 btc_users_addresses의 pending_received_balance_krw 에 더해준다. 
			} else if($btc_ads->currency == 'BTC') {			
				$this->remove_pending_btc($buyer_uid, $pending_btc_amt_with_fee); // buy_COIN_amt * buy_coin_price 만큼 uid 유저의 btc_users_addresses의 pending_received_balance_btc 에 더해준다. 
			} else if($btc_ads->currency == 'USD') {		
				$this->remove_pending_usd($buyer_uid, $pending_usd_amt_with_fee); // buy_COIN_amt * buy_coin_price 만큼 uid 유저의 btc_users_addresses의 pending_received_balance_usd 에 더해준다. 
    ...
    function Market_cancel_sell_order() {//판매요청 취소
        ...
        $btc_adss = DB::table('btc_ads')->where('type','sell')->where('status','CancelRequest')
                    ->where('currency',$process_currency)->where('sell_COIN_amt','>',0)->get();
        ...
        foreach($btc_adss as $btc_ads){
            ...
            $log_text = $log_text."\n\n";
            $log_text = $log_text."판매취소 IDX: ".$btc_ads->id."\n";
            $log_text = $log_text."판매취소 유저: ".$btc_ads->userid."\n";
            $log_text = $log_text."팬딩COIN 반환: ".$pending_COIN_amt."\n";

            DB::table('btc_ads')->where('id',$btc_ads->id)->update([
                "status" => 'Cancel',
            ]);
			info('취소하는 판매 '.$trade_coin_symbol.' 금액 : '.$pending_COIN_amt);
