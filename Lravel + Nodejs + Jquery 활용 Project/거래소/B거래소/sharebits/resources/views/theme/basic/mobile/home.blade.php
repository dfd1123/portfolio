    ...
    <div class="m_coin_chart">
        ...
        <div class="market_list_tab_bar">
            <ul class="market_list_tab">
                <li class="active">
                    <label for="ucss_market_list">UCSS마켓</label>
                </li>
                <li>
        ...
        <div class="coin_table_wrap">
            ...
            <table id="coin_table_usd" class="coin_chart_tbl target coin_table-1">
                <tbody>
                @forelse($coins as $coin)
                    @if($coin->percent_change_24h_usd >= 0)
                    <tr id="row_{{$coin->api}}" onclick="window.location.href = '{{route('marketUCSS', $coin->api)}}';" name="{{ __('main.m_'.$coin->api)}}/{{$coin->api}}">
                        <td class="coin_td"><p class="coin_name">{{__('coin_name.'.$coin->api)}}</p><span class="coin_name_eng">{{$coin->symbol}}/<b>UCSS</b></span></td>
            ...
            <table id="coin_table_btc" class="coin_chart_tbl target coin_table-2">
                <tbody>
                @forelse($coins as $coin)
                    @if($coin->api != 'btc')
            ...
            <table id="coin_table_eth" class="coin_chart_tbl target coin_table-3">
                <tbody>
                @forelse($coins as $coin)
                    @if($coin->api != 'eth')
