		...
		<div class="trans_con">
			...
			<div class="my_asset_wrap">
				...
				<div class="my_asset_con_wrap transac_con_wrap">
					...
					<div class="my_asset_con my_asset_1 toggle_con">
						...
						<div class="ast_info">
							...
							<ul>
								...
								<li>
									...
									<span class="align_right"> <b>{{ number_format($total_buying,8) }}</b> <u>UCSS</u> </span>
								</li>
								<li>
									<span>{{ __('my_asset.all_rating_revenue') }}</span>
									<span class="align_right"> <b>{{ number_format($total_eval_revenue,8) }}</b> <u>UCSS</u> </span>
								</li>
								<li>
									<span>{{ __('my_asset.my_ucss') }}</span>
									<span class="align_right"> <b>{{ number_format($coin_balance_usd,8) }}</b> <u>UCSS</u> </span>
								</li>
								<li>
									<span>{{ __('my_asset.all_rating_price') }}</span>
									<span class="align_right"> <b>{{ number_format($total_holding,8) }}</b> <u>UCSS</u> </span>
								</li>
								<li>
								...
								<span class="align_right">
								<b>{{ number_format($total_asset,8) }}</b>
								<u>UCSS</u>
								</span>
								</li>
								...
								@foreach($coins as $coin)
									...
									<tr name="{{ __('coin_name.'.$coin->api) }}">
										<td class="coin_td"><img class="coin_symbol" src="{{ asset('/storage/image/homepage/coin_img/'.$coin->api.'.png')}}" alt="coin_img"><span class="coin_name">{{ __('coin_name.'.$coin->api) }}</span><span class="coin_name_eng">{{ $coin->api }}/UCSS</span></td>
										<td>
											<p>
										...
										<td>
											<p>
												<span>{{ number_format($result[$coin->api]['avg'],8) }}</span>
												<span class="currency">UCSS</span>
											</p>
										</td>
										<td>
											<p>
												<span>{{ number_format($result[$coin->api]['buying'],8) }}</span>
												<span class="currency">UCSS</span>
											</p>
										</td>
										<td>
											<p>
												<span>{{ number_format($result[$coin->api]['eval'],8) }}</span>
												<span class="currency">UCSS</span>
											</p>
										</td>
										<td>
											...
											<p>
												{{ number_format($result[$coin->api]['eval_revenue'],8) }}<span class="currency" >UCSS</span>
											</p>
										</td>
										<td>
											<button class="status_btn" onclick="location.href='{{ route('marketUCSS',$coin->api) }}'">
											{{ __('my_asset.order') }}
											</button>
										</td>
									</tr>
					...
					<div class="my_asset_con history_con my_asset_2 toggle_con">
								...
								<tbody  id="history_lists">
									@forelse($historys as $history)
										...
										<tr class="trs_type-self" name="{{__('coin_name.'.$history->cointype)}}/{{$history->cointype}}">
											...
											<td>
												<p>
													<span>{{ strtoupper($history->currency) != 'USD' ? strtoupper($history->currency) : 'UCSS' }}{{ __('my_asset.market') }}</span>
												</p>
											</td>
											<td class="coin_td">
												...
												<span class="coin_name_eng">{{ strtolower($history->cointype) }}/{{ strtoupper($history->currency) != 'USD' ? strtoupper($history->currency) : 'UCSS' }}</span>
											</td>
											<td>
												<p>
										...
										<tr class="trs_type-buy">
											...
											<td class="coin_td"><img class="coin_symbol" src="{{ asset('/storage/image/homepage/coin_img/'.strtolower($history->cointype).'.png')}}" alt="coin_img"><span class="coin_name">{{ __('coin_name.'.strtolower($history->cointype)) }}</span><span class="coin_name_eng">{{ strtolower($history->cointype) }}/{{ strtoupper($history->currency) != 'USD' ? strtoupper($history->currency) : 'UCSS' }}</span></td>
											<td>
												<p>
													@if($history->buyer_uid == $history->seller_uid)
					...
					<div class="my_asset_con my_asset_3 toggle_con" >
								...
								<tbody id="not_concluded_lists">
									@forelse($wait_trades as $wait_trade)
										...
										<tr class="trs_type-buy">
											...
											<td>
												<p>
													<span>{{ strtoupper($wait_trade->currency) != 'USD' ? strtoupper($history->currency) : 'UCSS' }}{{ __('my_asset.market') }}</span>
												</p>
											</td>
											<td class="coin_td">
												<img class="coin_symbol" src="{{ asset('/storage/image/homepage/coin_img/'.strtolower($wait_trade->cointype).'.png')}}" alt="coin_img">
												<span class="coin_name">{{ __('coin_name.'.strtolower($wait_trade->cointype)) }}</span>
												<span class="coin_name_eng">{{ strtolower($wait_trade->cointype) }}/{{ strtoupper($wait_trade->currency) != 'USD' ? strtoupper($wait_trade->currency) : 'UCSS' }}</span>
											</td>
											<td>
											<p>
												<span>{{ __('my_asset.buy') }}</span>
