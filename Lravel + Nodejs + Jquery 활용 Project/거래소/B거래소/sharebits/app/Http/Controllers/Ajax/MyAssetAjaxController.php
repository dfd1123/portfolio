...
class MyAssetAjaxController extends Controller
{
    ...
    public function show_more_history(Request $request){
		...
		->where(function ($query) use ($uid){
			$query->where('buyer_uid',$uid)->orwhere('seller_uid',$uid);
		})->whereBetween('created_dt',[$start_date, $end_date])->select('btc_trades_COIN_btc.*','btc_coins.decimal_usd','btc_coins.decimal_btc','btc_coins.decimal_eth')->orderBy('id','desc')->offset($offset)->limit(10)->get();
		...
		foreach($historys as $history){
			...
			if($history->buyer_uid == $history->seller_uid){
				...
				$result[$i]['created_dt'] = $history->created_dt;
				...
				$result[$i]['total_price'] = number_format(bcadd($history->trade_usd_amt_sell,bcmul(bcmul($history->trade_usd_amt_sell,$setting->sell_comission,$history->{'decimal_'.strtolower($history->currency)}),"0.01",$history->{'decimal_'.strtolower($history->currency)}), $history->{'decimal_'.strtolower($history->currency)}), $history->{'decimal_'.strtolower($history->currency)});
				$result[$i]['fee'] = number_format(bcmul(bcmul($history->trade_usd_amt_sell,$setting->sell_comission,$history->{'decimal_'.strtolower($history->currency)}),"0.01",$history->{'decimal_'.strtolower($history->currency)}), $history->{'decimal_'.strtolower($history->currency)});
				$result[$i]['calcul_price'] = number_format($history->trade_usd_amt_sell, $history->{'decimal_'.strtolower($history->currency)});
			}
			$i++;
    ...
    public function search_date_history(Request $request){
		...
		->where(function ($query) use ($uid){
			$query->where('buyer_uid',$uid)->orwhere('seller_uid',$uid);
		})->whereBetween('created_dt',[$start_date, $end_date])->select('btc_trades_COIN_btc.*','btc_coins.decimal_usd','btc_coins.decimal_btc','btc_coins.decimal_eth')->orderBy('id','desc')->limit(20)->get();
    ...
    public function show_more_not_concluded(Request $request){
		...
		->where(function($qry) {
			$qry->where('sell_COIN_amt', '>', 0)->orWhere('buy_COIN_amt', '>', 0);
		})->select('btc_ads_btc.*','btc_coins.decimal_usd','btc_coins.decimal_btc','btc_coins.decimal_eth')->orderBy('id','desc')->offset($offset)->limit(10)->get();
		...
		foreach($wait_trades as $wait_trade){
			...
			if($wait_trade->type == 'buy'){
				...
				$result[$i]['created_dt'] = $wait_trade->created_dt; //만든날짜
    ...
    public function refresh_not_concluded(Request $request){
		...
		->where(function($qry) {
			$qry->where('sell_COIN_amt', '>', 0)->orWhere('buy_COIN_amt', '>', 0);
		})->select('btc_ads_btc.*','btc_coins.decimal_usd','btc_coins.decimal_btc','btc_coins.decimal_eth')->orderBy('id','desc')->limit(20)->get();
