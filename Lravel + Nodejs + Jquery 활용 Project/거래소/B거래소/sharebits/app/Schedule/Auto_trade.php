...
class Auto_trade
{
    public static function auto_trading() {
    	$coins = DB::table('btc_coins')->select('btc_coins.symbol','btc_coins.decimal_usd','btc_auto_setting.*')->leftJoin('btc_auto_setting','btc_coins.api','=','btc_auto_setting.cointype')
		...
		foreach($coins as $coin){
			...
			if(1 == 1){
				...
				$user_current_cash_balance = My_info::get_user_balance_allcoin("5269","usd");
				...
				if($type == 'buy'){
					$coin_address = My_info::walletinfo("5269", "address_btc");
				
					$trade_buy_krw_amt = "0";
					$trade_buy_usd_amt = bcmul($current_coin_price,$randnum_amt,17);
					$trade_buy_btc_amt = "0";
					$trade_buy_amt = bcmul($current_coin_price,$randnum_amt,17);
					
					$buy_coin_amt = $randnum_amt;
					
					$mul_current_range = bcmul($current_coin_price,$randnum_range,$coin->decimal_usd);
					$mul_reduce_current = bcmul($mul_current_range,0.01,$coin->decimal_usd);
					$buy_coin_price = bcadd($current_coin_price, $mul_reduce_current,$coin->decimal_usd);
					
					//$buy_coin_price = bcadd($current_coin_price,(bcmul(bcmul($current_coin_price,$randnum_range,$coin->decimal_usd),0.001,$coin->decimal_usd),$coin->decimal_usd);
					...
					$mul_current_range = bcmul($current_coin_price,$randnum_range,$coin->decimal_usd);
					$mul_reduce_current = bcmul($mul_current_range,0.01,$coin->decimal_usd);
					$sell_coin_price = bcadd($current_coin_price, $mul_reduce_current,$coin->decimal_usd);
					
					//$sell_coin_price = bcadd($current_coin_price,(bcmul(bcmul($current_coin_price,$randnum_range,$coin->decimal_usd),0.001,$coin->decimal_usd),$coin->decimal_usd);
					
					
					$trade_sell_krw_amt = "0";
					$trade_sell_usd_amt = bcmul($sell_coin_price , $sell_coin_amt,17);
					...
					//else if(($sell_coin_price > $current_coin_price*10 || $sell_coin_price < $current_coin_price*0.1) && $type == 'sell') {  $data = 5;   } // 시세보다 10% 이상 비싸게 팔려고 했을경우
						...
						echo "판매한 총가격 : ".$trade_sell_usd_amt."\n";
					}
	...
	public static function auto_cancel() {
		DB::table('btc_ads')->where('created','<',DB::raw('UNIX_TIMESTAMP(NOW() - INTERVAL 48 HOUR)'))->where('userid','sbtr01')->where('status','OnProgress')
		->where(function ($query){
