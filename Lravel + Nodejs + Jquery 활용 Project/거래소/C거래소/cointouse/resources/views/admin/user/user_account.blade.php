                  ...
                  @forelse($securitys as $security)
                    <tr>
                      ...
                      <td>
                      	...
                      	@else
                      		<button type="button" class="account_view" data-src="{{asset($security->document_1)}}|{{asset($security->account_1)}}|{{asset($security->account_2)}}">{{ __('user.check') }}</button>
                      	@endif
                      </td>
                      <td>
                      ...
                      <td>
                      	@if($security->account_verified == 0 && $security->account_reject != NULL)
                      		<button type="button" class="myButton account_reject_load_btn" data-id="{{$security->uid}}">{{ __('user.reason') }}</button>
                      	@elseif($security->account_verified == 2)
                      		<button type="button" class="myButton navy account_agree_btn" data-id="{{$security->uid}}">{{ __('user.yes') }}</button>
                      		<button type="button" class="myButton xbtn account_reject_load_btn" data-id="{{$security->uid}}">{{ __('user.nope') }}</button>
                      	@elseif($security->account_verified == 1)
                      		<button type="button" class="myButton xbtn account_reject_load_btn"  data-id="{{$security->uid}}">{{ __('user.nope') }}</button>
                      	@endif
                      </td>
                    </tr>
			...
			<div class="jw_modal_content">
				...
				<div class="jw_modal_bd">
					<div class="content_box">
						...
						<div style="width:33%;float:left;margin-right:0.3%;">
							통장사본
							<img id="view_account1" src="" style= "width:100%;">
						</div>
						<div style="width:33%;float:left;margin-right:0.3%;">
							통장 들고있는 사진
							<img id="view_account2" src="" style= "width:100%;">
						</div>
					</div>
				</div>
			</div>
