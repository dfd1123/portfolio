			...
			<table class="table table-bordered" width="100%" cellspacing="0">
				<thead>
					<tr>
						...
						<th style="width:10%;">코인 이름</th>
						<th style="width:10%;">봇 보유량</th>
						<th style="width:10%;">유저 보유량</th>
						<th style="width:10%;">봇 + 유저 보유량</th>
						<th style="width:10%;">거래소 보유량</th>
						<th style="width:10%;">회사 순자산</th>
				...
				<tbody>
					...
					<tr>
						...
						<td>{{ $results['company_balance_'.$coin->api] }} {{ $coin->symbol }}</td>
						<td>{{ $results['users_balance_'.$coin->api] }} {{ $coin->symbol }}</td>
						<td>{{ $results['company_balance_'.$coin->api] + $results['users_balance_'.$coin->api] }} {{ $coin->symbol }}</td>
						<td>{{ $results['market_balance_'.$coin->api] }} {{ $coin->symbol }}</td>
						<td>{{ $results['market_balance_'.$coin->api] - ( $results['users_balance_'.$coin->api] + $results['company_balance_'.$coin->api] ) }} {{ $coin->symbol }}</td>
						@if($results['market_balance_'.$coin->api] >= $results['company_balance_'.$coin->api] + $results['users_balance_'.$coin->api] )
						<td class="status"><span class="blue">정상</span></td>
						@else
						<td class="status"><span class="red">비정상</span></td>
						@endif
                        <!--td>-</td-->
					</tr>
					@empty
