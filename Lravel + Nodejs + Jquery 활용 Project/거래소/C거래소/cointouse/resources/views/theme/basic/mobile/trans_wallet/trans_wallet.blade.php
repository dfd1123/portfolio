    ...
    <div class="left_wrap both_wrap">
        ...
        <div class="status_bar">
            <div class="total_ast text-right">
                ...
                <span class="point_clr_1">{{ number_format($total_holding,0) }}</span>
            ...
            <div class="coin_table">
                <table class="coin_chart_tbl" id="coin_list_table">
                    ...
                    @foreach($coins as $coin)
						...
						<tr name="USDC" onclick = "select_coin('{{ $coin->symbol }}');" class = "{{ $result[$coin->api]['balance'] > 0 ? 'exist_balance' : '' }}">
                            <td class="coin_td">
                                <img src="{{ asset('/storage/image/homepage/coin_img/usdc.png') }}" alt="coin_img" class="coin_img" />
                                <span class="coin_name">USDC</span>
                            </td>
                            <td>
						...
						<tr name="{{ __('coin_name.'.$coin->api) }}/{{$coin->api}}" onclick = "select_coin('{{ $coin->symbol }}');" class = "{{ $result[$coin->api]['balance'] > 0 ? 'exist_balance' : '' }}">
                            ...
                            <td class="td_btn">
                                <button onclick="exchangeCashCoin('KRW','USD');">{{ __('trans.change_usdc') }}</button>
                            </td>
        ...
        <div class="status_bar">
            <h1>
                ...
                <label class="ntc_label" for="ntc_box">{{ __('trans.ue') }}
                    ...
                    <span class="ntc_box_ment">
                        ...
                        <span>
                        - 실제로 전송되는 암호화폐 총 수량은 신청한 수량에 전송 수수료를 제외한 수량입니다.(단, 코인투스 회원간의 송금시에는 수수료가 발생하지 않습니다.)
                        </span><br><br>
                        <span>
                        ...
                        <span>
                        - 출금신청 완료 이후의 송금 과정은 블록체인 네트워크에서 처리됩니다. 이 과정에서 발생할 수 있는 송금 지연 등의 문제는 코인투스에서 처리가 불가능합니다.
                        </span><br><br>
                        <span>
            ...
            <div class="total_ast text-right">
                ...
                <p class="s_size">
                    ...
                    <span id="this_balance_pending"> <u>KRW</u></span>
                </p>
                <p class="s_size">
                    <label class="float-left">{{ __('trans.can_out') }}</label>
                    <span id="this_balance_available"> <u>KRW</u></span>
                </p>
            </div>
        </div>
        <!-- //위에 유의사항 및 코인정보들 -->
        ...
        <div class="scrl_wrap">
            ...
            @else
                ...
                <div class="trans_cash_table hide">
                    <!-- START 1.입금 -->
                    <div class="con_1 con_box">
                        <span class="tit mt-4" id="first_deposit_info">{!! __('trans.cash_sentence1') !!}</span>

                        <input class="qr_adrs mb-3 form-control" type="number" id="cash_deposite" placeholder="Click Here!" />

                        <button type="button" class="mini_btn_st transwallet_btn" onclick="cash_deposite();">{{__('trans.in')}}</button>
                    </div>
                    <!-- //END 1.입금 -->
                    ...
                    <div class="con_2 con_box">
                        <div class="form_group">
                            <label>{{ __('trans.one_day_out_limit') }}</label>
                            <span class="red float-right" id="cash_withdraw_limit_amt">
                                <b>0</b> <b class="currency">KRW</b>
                            </span>
                    </div>
                    ...
                    <div class="form_group_wrap bb-f0f0 bt-f0f0 pt-2 pb-1">
                        <div class="form_group mb-3">
                            ...
                            <div class="form_group ta_form_group">
                                <label
                                    ...
                                    <span class="mini_btn_st" onclick="withdraw_max_amt();">{{ __('trans.max') }}</span></label
                                >
                                <input type="number" class="form-control"  id = "cash_withdraw_amt"  onchange="withdraw_onkey_amt();"/>
                            </div>
                        </div>
                        ...
                        <div class="form_group bb-f0f0 pt-2 pb-2">
                            <label>{{ __('trans.out_fees') }}</label>
                            <span class="float-right">
                                <b id="withdraw_send_fee">0.00000000</b> <b class="currency" id="cash_withdraw_send_fee"></b>
                            </span>
                        </div>
                        ...
                        <div class="form_group mb-4 lg_font">
                            <label>{{ __('trans.out_fees') }}</label>
                            <span class="float-right">
                                <b id="withdraw_total_amt">0.00000000</b> <b class="currency" id="cash_withdraw_total_amt"></b>
                            </span>
                        </div>
