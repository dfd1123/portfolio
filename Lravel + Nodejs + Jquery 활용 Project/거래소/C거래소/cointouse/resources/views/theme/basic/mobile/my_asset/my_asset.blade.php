                ...
                <div class="status_bar">
                    ...
                    <div class="total_ast">
                        <p class="text-right">
                            ...
                            <span class="point_clr_1">{{ number_format($coin_balance_krw,0) }}</span>
                            <span class="currency">KRW</span>
                        </p>
                        <p class="text-right">
                            <label class="float-left tit">{{ __('my_asset.all_my_money') }}</label>
                            <span class="point_clr_1">{{ number_format($total_asset,0) }}</span>
                            <span class="currency">KRW</span>
                        </p>
                    </div>
				...
				@foreach($coins as $coin)
	                ...
	                <div class="my_coin_list_con {{ $result[$coin->api]['balance'] > 0 ? 'exist_balance' : '' }}" >
	                    ...
	                    <div class="hd dp_table ">
	                        ...
	                        <div class="dp_table_cell _left"  onclick="location.href='{{ route('marketKRW',$coin->api) }}'">
                    ...
                    @forelse($historys as $history)
                    	...
                    	<li class="con self" name="{{__('coin_name.'.$history->cointype)}}/{{$history->cointype}}">
                            <p class="info _date mb-2">
                                <span>{{ date("m-d H:i:s", strtotime("+9 hours", strtotime($history->created_dt))) }}</span>
                                <span class="float-right type">{{ __('my_asset.self_trade') }}</span>
                            </p>
                            <p class="info _coin">
                                <span>{{ strtoupper($history->currency) != 'USD' ? strtoupper($history->currency) : 'USDC' }}마켓</span>
                ...
                <div class="history_st">
                    <ul class="list" id = "not_concluded_lists">
                    	@forelse($wait_trades as $wait_trade)
	                        ...
	                        <li class="con buy">
	                            <p class="info _date mb-2">
	                                <span>{{ date("m-d H:i:s", strtotime("+9 hours", strtotime($wait_trade->created_dt))) }}</span>
	                                <span class="float-right type">{{ __('my_asset.gm') }}</span>
	                            </p>
	                            <p class="info _coin">
	                            	<span>{{ strtoupper($wait_trade->currency) != 'USD' ? strtoupper($wait_trade->currency) : 'USDC' }}마켓</span>
