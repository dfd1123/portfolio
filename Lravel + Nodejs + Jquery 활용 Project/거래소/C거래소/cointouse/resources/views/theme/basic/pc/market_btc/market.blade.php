				...
				<div class="inner_box right_con_inbox-1">
					<div class="tit">
						<ul class="market_list_tab">
							<li {{ $symbol_text == 'KRW' ? ' class=active ' : '' }} >
								<label for="krw_market_list">
									KRW{{ __('market.market') }}
								</label>
							</li>
							<li {{ $symbol_text == 'USDC' ? ' class=active ' : '' }} >
								<label for="usdc_market_list">
									USDC{{ __('market.market') }}
								</label>
							</li>
							{{--
					...
					<div class="scl_wrap">
						<input id="usdc_market_list" class="hide" type="radio" name="coin_list" {{ $symbol_text == 'USDC' ? 'checked=checked' : '' }} />
						...
						<input id="krw_market_list" class="hide" type="radio" name="coin_list" {{ $symbol_text == 'KRW' ? 'checked=checked' : '' }} />
						<table id="coin_list_table_usd" class="table_in_box coin_table target coin_table-1">
							<tbody>
								@foreach($coins as $coin)
									<tr name="{{__('coin_name.'.$coin->api)}}" onclick="location.href='{{route('marketUSDC',$coin->symbol)}}'" data-coin="{{$coin->api}}">
										<td>
						...
						<table id="coin_list_table_eth" class="table_in_box coin_table target coin_table-3">
						 ...
						 --}}
						{{-- KRW마켓 --}}
						<table id="coin_list_table_krw" class="table_in_box coin_table target coin_table-4">
							<tbody>
								@foreach($coins as $coin)
									@if($coin->api != 'krw')
									<tr name="{{__('coin_name.'.$coin->api)}}/{{$coin->api}}" onclick="location.href='{{route('marketKRW',$coin->symbol)}}'" data-coin="{{$coin->api}}">
										<td>
										<img src="{{ asset('/storage/image/homepage/coin_img/'.strtolower($coin->image))}}" alt="{{__('coin_name.'.$coin->api)}}" class="coin_img"/>
										<p>
											{{__('coin_name.'.$coin->api)}}
										</p><span>({{$coin->symbol}}/KRW)</span></td>
