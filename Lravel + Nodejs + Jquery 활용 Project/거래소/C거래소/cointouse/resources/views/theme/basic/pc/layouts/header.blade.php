<nav class="navbar-sharebits">
	...
	<div class="nav_container">
		...
		<a href="{{ url('/?country='.config('app.country')) }}" class="logo"> <img src="{{ asset('/images/header_cointouse.svg')}}" alt="logo"/> </a>

		<ul class="center_nav">

			<li class="{{request()->is('market*') ? 'active' : ''}} main_menu">
				<a href="{{ route('marketKRW','btc') }}">{{ __('head.trademarket') }} </a>
			</li>
			...
			<li class="{{request()->is('ico*') || request()->is('my_ico*') ? 'active' : ''}} main_menu drop_menu">
				<a href="{{ route('ico_list','all') }}">ICO/IEO</a>
				<ul class="drop_down">
					<li>
						<a href="{{ route('ico_list','all') }}">ICO/IEO</a>
					</li>
