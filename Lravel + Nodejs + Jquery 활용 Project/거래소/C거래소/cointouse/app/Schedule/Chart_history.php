	...
	public static function update_market_info_new() {
		...
		foreach($coins as $coin){
			...
			DB::table('btc_coins')->where('symbol', strtoupper($coin->api))-> update([
	            ...
	            'percent_change_24h_krw' => DB::raw("IFNULL(( price_krw - IFNULL( (SELECT `buy_coin_price` as opening_price FROM `btc_trades_COIN_btc` WHERE `cointype` = '".$coin->api."' and currency = 'krw' and `created` >  UNIX_TIMESTAMP( NOW() - INTERVAL 24 HOUR ) order by id asc limit 1 ),price_krw)  )* 100 / NULLIF(price_krw,0) , 0)"),
