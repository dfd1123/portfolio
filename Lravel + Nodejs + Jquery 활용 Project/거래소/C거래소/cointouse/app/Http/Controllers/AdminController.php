...
class AdminController extends Controller {
    public function index() {
        ...
        $month_trades_revenues = DB::select(DB::raw("select btc_trades_COIN_btc.currency, CONCAT(YEAR(btc_trades_COIN_btc.created_dt),'-',MONTH(btc_trades_COIN_btc.created_dt)) AS date, (SUM(btc_trades_COIN_btc.contract_coin_amt*btc_trades_COIN_btc.buy_coin_price) * (SELECT (buy_comission + sell_comission) AS fee FROM btc_settings WHERE id = 1 ) * 0.01) - IFNULL((SELECT sum(amount) FROM btc_lock_dividend where CONCAT(YEAR(created_dt),'-',MONTH(created_dt)) = date),0) AS total_fee_price from `btc_trades_COIN_btc` where `btc_trades_COIN_btc`.`created` > UNIX_TIMESTAMP(NOW() - INTERVAL 1 YEAR) and `buyer_username` <> 'sbtr01' and `seller_username` <> 'sbtr01' group by btc_trades_COIN_btc.currency, CONCAT(YEAR(btc_trades_COIN_btc.created_dt),'-',MONTH(btc_trades_COIN_btc.created_dt)) order by CONCAT(YEAR(btc_trades_COIN_btc.created_dt),'-',MONTH(btc_trades_COIN_btc.created_dt)) asc"));

        $month_withdraws_revenues = DB::select(DB::raw('select CONCAT(YEAR(created_dt),"-",MONTH(created_dt)) AS date, (SUM(send_fee)* IFNULL((SELECT avg(btc_trades_COIN_btc.buy_coin_price) FROM `btc_trades_COIN_btc` WHERE currency = "USD" and CONCAT(YEAR(btc_trades_COIN_btc.created_dt),"-",MONTH(btc_trades_COIN_btc.created_dt)) = date and btc_coin_send_request.cointype = btc_trades_COIN_btc.cointype ),0) ) AS amt, `cointype` from `btc_coin_send_request` where `created` > UNIX_TIMESTAMP(NOW() - INTERVAL 1 YEAR) and status = "withdraw_complete" group by `cointype`, CONCAT(YEAR(created_dt),"-",MONTH(created_dt)) ORDER BY `btc_coin_send_request`.`cointype` ASC'));
    ...
    public function user_excel(Request $request){
        ...
        return Excel::download(new UsersExport($from, $to), '쉐어비츠사용자리스트_' .  date("Y-m-d") . '.xlsx'); 
    }
	...
	public function user_detail(Request $request, $uid, $tab){
        ...
        if($tab == 1){
        	...
        	$qnas = DB::table('html.users')
                    ->leftJoin('html_read.btc_qna','html_read.btc_qna.createdby','=','html.users.username')
    ...
    public function coin_tr_excel(Request $request){
        ...
        return Excel::download(new CoinTrsExport($from, $to), '쉐어비츠입출금이력리스트_' .  date("Y-m-d") . '.xlsx'); 
    }
    ...
    public function trade_history_excel(Request $request){
        ...
        return Excel::download(new TradeHistorysExport($from, $to), '쉐어비츠거래이력리스트_' .  date("Y-m-d") . '.xlsx'); 
    }
    ...
    public function coin_has_list(){
		...
		foreach($coins as $coin){
			...
			if($coin->cointype == 'coin' && $coin->symbol != 'ETH'){
			    ...
			    $url = $setting->url_io."api/get_private_info.php"; // Where you want to post data
			    info($url);
				...
				if($coin->symbol == 'ETH'){
					$ch = curl_init();                    // Initiate cURL
				    $api_url = 'https://api.etherscan.io/api?module=account&action=balance&address=0x42FAf1348649c204BEF4285FdDA498982183676D&tag=latest&apikey=X59FEHDBQKKI8P6NHQ3J5IPFXEYCQ42RI3';
					...
					$ch = curl_init();                    // Initiate cURL
				    $api_url = 'https://api.etherscan.io/api?module=account&action=tokenbalance&contractaddress='.$coin->token_contract_addr.'&address=0x42FAf1348649c204BEF4285FdDA498982183676D&tag=latest&apikey=X59FEHDBQKKI8P6NHQ3J5IPFXEYCQ42RI3';
				    curl_setopt($ch, CURLOPT_URL,$api_url);
					curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
					curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
				    $result = curl_exec ($ch); // Execute
