...
<div class="card-body">
	<div class="usr_search_box tsa-sch-box">
		<form method="get" action="">
				<select name="keyword_srch">
					...
					<option value="id">{{ __('user.email') }}</option>
	...
	<table class="table table-bordered " id="dataTable" width="100%" cellspacing="0">
		<thead>
			...
			<th style="width: 7%;">사용자</th>
			<th style="width: 7%;">{{ __('user.name') }}</th>
			<th style="width: 15%;">{{ __('user.email') }}</th>
			<th style="width: 20%;">{{ __('user.ucss') }}</th>
			<th style="width: 20%;">{{ __('user.wait_ucss') }}</th>
		...
		<tr>
			...
			<td>{{$user->available_balance_usd}}</td>
			<td>{{$user->pending_received_balance_usd}}</td>
			<td>
		...
		<div class="jw_modal_content">
			...
			<div class="jw_modal_bd">
				<div class="content_box">
					...
					<div class="section_div">
						<h5><i class="fal fa-chevron-circle-right pr-2"></i>{{ $coin->name }}</h5>
...
<script>
	...
	loadPopup('.availble_coin_confirm', '#availble_confirm_wrap', function(e) {
		...
		$.ajax({
			...
			success : function(data) {
				@foreach($coins as $coin)
					$('input[name="available_balance_{{$coin->api}}"]').val(data.available_balance_{{$coin->api}});
					$('input[name="pending_balance_{{$coin->api}}"]').val(parseFloat(data.available_balance_{{$coin->api}}) + parseFloat(data.pending_received_balance_{{$coin->api}}));
				@endforeach
			}
		});
	});
