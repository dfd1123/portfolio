    ...
    public function index()
    {
		...
		foreach($coins as $coin){
			...
			if($coin->symbol == config('app.default_cash')){
				$coin_balance_usd = bcadd($coin_balance,0,8); // 보유 ucss 양
				continue;
			}
			...
			$total_buying = bcadd($total_buying,$coin_buying_price,8); //총 매수금액
			...
			$result[$coin->api]['eval_revenue'] = bcsub($coin_balance_price,$coin_buying_price,8); //코인별 평가 수익
			$result[$coin->api]['eval_percent'] =  $coin_buy_percent; //코인별 평가손익
		}

		if($total_buying != 0){ //코인 총 매수금액이 0일때 0으로 다 대체
			$total_eval_revenue = bcsub($total_holding,$total_buying,8); //총 평가수익
		...
		$views->coin_balance_usd = $coin_balance_usd; //보유 ucss 자산
		$views->total_asset = bcadd($total_holding, $coin_balance_usd,8); //총 보유자산
