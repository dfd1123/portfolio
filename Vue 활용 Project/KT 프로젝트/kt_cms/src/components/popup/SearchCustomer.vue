<template>
  <div
    class="layer-pop layer-pop-parents customer-search"
    :class="align ? align : ''"
  >
          ...
          >
            <td>
              <p class="check-type01 single">
                <input
                  type="radio"
                  :id="status + 'check_'+item.custId"
                  :value="item.custId"
                  v-model="radioValue"
                  class="single_expert_check"
                ><label :for="status + 'check_'+item.custId"><span></span></label>
              </p>
            </td>
            <td>{{ item.custId }}</td>
            <td>{{ item.custNm || '-' }}</td>
...
<script>
  ...
  export default {
    ...
    computed: {
      selectedItem () {
        return this.list.find((x) => x.custId === this.radioValue)
      }
    },
    created () {
      setTimeout(() => {
    ...
    methods: {
      async GetData () {
        ...
        setTimeout(() => {
          this.list.forEach(x => {
            ...
            this.$http
              .get(this.$APIURI + 'common/v1/service', { cancelToken: source.token, params })
              .then(res => res.data)
              .then(res => {
                const data = this.FirstData(res.data, null)
                if (data) {
                  Vue.set(x, 'svcNm', data.svcNm)
                  Vue.set(x, 'svcSttusCd', data.svcSttusCd)
                }
              }).catch(() => { })
          })
        }, 250)
      ...
      layerPopClose: function () {
        window.$('.customer-search').stop(true).fadeOut(350)
      }
    }
  }
</script>
