<template>
  <div
    class="layer-pop layer-pop-parents user-search"
    :class="align ? align : ''"
  >
    ...
    methods: {
      async GetData () {
        ...
        if (this.onlyAdmin) {
          ...
          // 중복제거
          const seen = new Set()
          const filteredList = res2.filter(x => {
            const duplicate = seen.has(x.userId)
            seen.add(x.userId)
            return !duplicate
          })
      ...
      layerPopClose: function () {
        window.$('.user-search').stop(true).fadeOut(350)
      }
    }
  }
</script>
