    ...
    if (localStorage.passportToken) {
        if (!store.state.detail.user) {
            try {
                ...
                store.commit("favors", datas[0]);
                store.commit("asset", datas[1]);
                store.commit("detail", datas[2]);
                store.commit("selectedCoinInfo", datas[3]);
                store.commit("selectedCoinHistorys", datas[4]);
            } catch (e) {
                localStorage.removeItem("passportToken");
                axios.defaults.headers.common["Authorization"] = undefined;
