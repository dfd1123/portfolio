...
class WalletController extends Controller
{
    ...
    public function buy_tru(Request $request){
        $coin = strtolower($request->symbol);
        $amount = $request->amount;
        $user = Auth::user();
        ...
        $eth_amount = bcmul($Settings->tru_per_eth, $amount,8); // 구매한 코인의 수량에 맞는 지불할 ETH량을 계산
        //dd('구매 금액'.$eth_amount);

        //값 검증
        if(!is_numeric($amount) && (float) $amount > 0) {
            return response()->json(["status" => 'is_numeric', "error" => 'Invalid Amount'], 422);
        }

        // 사용자 ETH 검증
        $balance = Wallet::balance($user->id, 'eth');
        if(bccomp($eth_amount, $balance, 0) == 1) { //$amount가 $balance보다 클 때
            return response()->json(["status" => 'over_balance', "error" => 'Invalid Request'], 422);
        }

        // 구매 유저의 잔액 변경
        $status = DB::table('btc_users_addresses')->where('uid', $user->id)->update([ 
            'available_balance_eth' => DB::raw('available_balance_eth - '.$eth_amount),
            'available_balance_'.$coin => DB::raw('available_balance_'.$coin.' + '.$amount)
        ]);

        //dd('123123');
        
        // 판매 유저(최고 관리자)의 잔액 변경
        if($status){
            $status = DB::table('btc_users_addresses')->where('uid', 1)->update([
                'available_balance_eth' => DB::raw('available_balance_eth + '.$eth_amount),
                'available_balance_'.$coin => DB::raw('available_balance_'.$coin.' - '.$amount)
            ]);
        }

        // 거래이력 테이블에 거래 정보를 등록(구매, 판매 두개 다)
        $insert_data = array(
            array(
                'cointxid' => $coin.'_trade_'.$amount.'_trustorn9_'.$username.'_buy',
                'cointype' => $coin,
                'account' => $username,
                'address' => 'trustorn9',
                'category' => 'trade',
                'amount' => $amount,
                'confirmations' => 999,
                'txid' => $coin.'_trade_'.$amount.'_trustorn9_'.$username.'_buy',
                'normtxid' => '',
                'tr_time' => time(),
                'timereceived' => time(),
                'processed' => 'y',
            ...
            array(
                'cointxid' => 'eth_internal_transfer_'.$eth_amount.'_'.$username.'_trustorn9_send',
                'cointype' => 'eth',
                'account' => $username,
                'address' => 'trustorn9',
                'category' => 'send',
                'amount' => $eth_amount,
                'confirmations' => 999,
                'txid' => 'eth_internal_transfer_'.$eth_amount.'_'.$username.'_trustorn9_send',
                'normtxid' => '',
                'tr_time' => time(),
                'timereceived' => time(),
                'processed' => 'y',
            ...
            array(
                'cointxid' => 'eth_internal_transfer_'.$eth_amount.'_'.$username.'_trustorn9_receive',
                'cointype' => 'eth',
                'account' => 'trustorn9',
                'address' => $username,
                'category' => 'receive',
                'amount' => $eth_amount,
                'confirmations' => 999,
                'txid' => 'eth_internal_transfer_'.$eth_amount.'_'.$username.'_trustorn9_receive',
                'normtxid' => '',
                'tr_time' => time(),
                'timereceived' => time(),
                'processed' => 'y',
        ...
        if($status){
            $status = DB::table('btc_transaction')->insert($insert_data);
        }
    ...
    public function user_revenue(){
        $revenue_lists = DB::table('monthly_personal_revenue')->where('uid', Auth::user()->id)->orderBy('created_at', 'desc')->get();

        return response()->json($revenue_lists, 200);
    }

    public function monthly_revenue(){
        $all_revenues = DB::table('monthly_all_revenue')->orderBy('created_at', 'desc')->limit(12)->get();

        return response()->json($all_revenues, 200);
    }
}
