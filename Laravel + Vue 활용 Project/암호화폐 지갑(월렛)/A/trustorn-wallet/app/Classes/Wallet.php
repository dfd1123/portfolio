...
class Wallet
{
    ...
    public function transaction($username, $symbol, $days, $offset = 0, $limit = PHP_INT_MAX)
    {
        ...
        foreach ($transactions as $transaction) {
            ...
            if ($username == $transaction->account && $transaction->category == 'send') {
                $status = 'send';
                $sign = '-';
                $from = $account_name;
                $to = $address_name;
            } elseif ($username == $transaction->account && $transaction->category == 'receive') {
                $status = 'receive';
                $sign = '+';
                $from = $address_name;
                $to = $account_name;
            ...
            //dd($status.'/'.$sign.'/'.$from.'/'.$to);

            if($transaction->category != 'trade'){
                if ($username == $transaction->account){
                    array_push($response, [
                        'id' => $transaction->id,
                        'symbol' => $symbol,
                        'from' => $from,
    ...
    public function check_address($symbol, $address)
    {
        ...
        $client = new Client();
        //dd($Settings);
        $res = $client->get($Settings->url_io . "isAccount/$address");
    ...
    private function create_new_address($username, $coin_type)
    {
        $Settings = Settings::Settings();
        $client = new Client();
        $res = $client->get($Settings->url_io."newAddr/".$username);
