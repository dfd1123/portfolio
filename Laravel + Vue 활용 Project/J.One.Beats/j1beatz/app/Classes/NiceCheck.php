...
class NiceCheck
{
    public function NiceCheck_main()
    {
        ...
        if (strpos(config('app.url'), "local") !== false) {
        ...
        $returnurl = config('app.url')."/nicecheck/checkplus_success";	// 성공시 이동될 URL
        $errorurl = config('app.url')."/nicecheck/checkplus_fail";		// 실패시 이동될 URL
    ...
    public function NiceCheck_success()
    {
        ...
        if (preg_match('~[^0-9a-zA-Z+/=]~', $enc_data, $match)) {
            info("입력 값 확인");
            return "입력 값 확인이 필요합니다 : ".$match[0];
        } // 문자열 점검 추가.
        if (base64_encode(base64_decode($enc_data))!=$enc_data) {
            info("입력 값 확인");
            return "입력 값 확인이 필요합니다";
        }
        ...
        if ($enc_data != "") {
            ...
            if ($plaindata == -1) {
                ...
                $returnMsg = $mobileno;
                return ["mobileno" => $mobileno,"name" => $name , "returnMsg" => "성공"];
            }
        }
    }
