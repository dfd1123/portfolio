...
class CommentController extends Controller
{
    ...
    public function destroy(Request $request, $comment){
        ...
        if($status){
            $status = DB::table('user_trd_comment')->where('ucc_no', $comment)->delete();
        }
    ...
    private function write(Request $request){
        ...
        if($ucc_no != ''){
            $status = DB::table('user_trd_comment')->where('ucc_no', $ucc_no)->where('client_no', auth()->user()->user_no)->update([
                "ucc_title" => $title,
		...
		if($index <= 2){
            ...
            if($ucc_no != ''){
                ...
                if($user_trd_comment->ucc_files == null || $user_trd_comment->ucc_files == '[]'){
                    ...
                    DB::table('user_trd_comment')->where('ucc_no', $ucc_no)->where('client_no', auth()->user()->user_no)->update([
                    ...
                    DB::table('user_trd_comment')->where('trd_no', $trd_no)->where('client_no', auth()->user()->user_no)->update([
        ...
        if(File::exists($img_path)) {
            if(File::delete($img_path)){
                if(count($ucc_imgs) == 0 && $user_trd_comment->ucc_files == NULL){
                    $status = DB::table('user_trd_comment')->where('ucc_no',$ucc_no)->delete();
                }else{
                    $status = DB::table('user_trd_comment')->where('ucc_no',$ucc_no)->update([
                        "ucc_imgs" => json_encode($ucc_imgs),
                    ]);
                }
            }
