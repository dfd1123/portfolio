<template>
  ...
  >
    ...
    >
      <!-- 1) 주문상품 확인리스트 구역 -->
      <div class="l-con-area full mb-35">
        <article class="l-con-article mb-15">
            ...
            <ul class="l-grid-group">
              ...
              >
                ...
                <CartWideItemList
                  ...
                  :coupon-id="orderList.coupon_id"
                  :coupon-subject="orderList.coupon_subject"
                  :coupon-price="orderList.coupon_price"
      ...
      <div class="dg-order-info-group clear_both">
        ...
        <div class="in-detail-info">
          ...
          <article class="l-con-article">
            <h4 class="in-subject">
              <span>배송준비 기간</span>
              <div class="_question_icon">
                <img
                  src="/images/icon/icon_question.svg"
                  alt="icon"
                >
                <div class="_paragraph_box">
                  <p>
                    동글은 묶음 배송 서비스를 기본으로 제공하여 고객님의 배송비용을 아껴드립니다.<br>
                    다만 도매의 특성상 재고가 없을 경우 상품 확보에 시일이 걸릴 수 있습니다.<br>
                    아래에 보이는 2일/5일/10일로 배송준비기간을 선택해 주시면, 선택한 기간 내에<br>
                    출고되지 못한 상품은 자동으로 주문취소 및 환불처리 됩니다.
                  </p>
                </div>
              </div>
            </h4>
            <div class="in-days-box">
              <span class="_days">
                <input
                  type="radio"
                  name="hopeDate"
          ...
          <article class="l-con-article">
            ...
            <ul class="in-information mb-30">
              ...
              <li class="_list">
                ...
                <p class="_txt">
                  {{ NumberFormat(form.sendCost) }} 원
                </p>
              </li>
              <li class="_list _list--sale display_none">
                <span class="_label">회원등급할인</span>
                <p class="_txt">
                  - {{ NumberFormat(levelDiscount) }} 원
                </p>
              </li>
              <li class="_list _list--sale">
                <span class="_label">보유쿠폰 할인</span>
                <p class="_txt">
                  - {{ NumberFormat(CouponDiscount) }} 원
                </p>
              </li>
              <li class="_list _list--final">
                <span class="_label">최종 결제금액</span>
                <p class="_txt">
                  <b>{{ NumberFormat(ReceiptPrice) }} 원</b>
                  <input
                    type="text"
                    name="Amt"
                    class="display_none"
            ...
            <ul class="in-agree-list">
              <li class="_agr_list">
                ...
                <label
                  ...
                  class="dg-input-checkbox_label"
                ></label>
                <label for="privacyAgree">개인정보 제3자 제공고지</label>
              </li>
              <li class="_agr_list">
                <input
                  type="checkbox"
                ...
                <label
                  ...
                  class="dg-input-checkbox_label"
                ></label>
                <label for="buyAgree">구매진행에 동의합니다.</label>
                <label
                  for="agree-info-more"
                  class="_more"
                >더보기<img
...
<script>
  ...
  import CouponPopup from '@/components/popup/coupon.vue'
  ...
  export default {
    ...
    async created () {
      ...
      if (!this.$store.state.user.account_bank || !this.$store.state.user.account_name || !this.$store.state.user.account_number) {
        const result = await this.WarningAlert('환불 받을 계좌정보가 등록되어 있지 않습니다.\n환불 계좌 등록 페이지로 이동합니다.')
        if (result) {
    ...
    methods: {
      ...
      AllCouponSubmit (value) {
        if (value) {
          ...
          this.allCouponLists.forEach((coupon) => {
      ...
      async nicepayStart () {
        window.goPay(document.niceForm)
      },
