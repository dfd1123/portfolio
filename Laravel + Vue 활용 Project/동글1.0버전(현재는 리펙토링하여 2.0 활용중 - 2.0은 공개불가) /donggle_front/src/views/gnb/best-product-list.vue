  ...
  >
    <div class="dg-gnb-choice-group">
      <div class="dg-gnb-title-group">
        ...
        <GnbTitle
          gnb-title="인기상품(BEST)"
          gnb-text="실시간으로 가장 많이 판매되는 상품을 소개드려요."
        />
        <!-- * 대메뉴 제목 E -->
        <ul class="in-breadcrumb type01">
          ...
          <li class="_list active">
            <a href="/product/list/best">인기상품(BEST)</a>
          </li>
        </ul>
      </div>
    </div>
        ...
        <div class="l-contents-group">
          ...
          >
            <img
              ...
              class="in-empty-icon"
            >
            <span class="in-empty-ment">인기상품(BEST)에 등록된 상품이 없습니다.</span>
          </div>
    ...
    methods: {
      async ItemLoad () {
        try {
          ...
          const res = (await this.$http.get(this.$APIURI + 'items/best_list', { params: this.form })).data
      ...
      async Submit () {
        ...
        this.$router.replace({ name: 'best-product-list', query: params })
        this.form.offset = 0
        const res = await this.ItemLoad()
        this.itemLists = res.query.items
        this.form.itemCount = res.query.count
