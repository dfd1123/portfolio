...
import QuickMenu from '@/components/common/quick_menu/quick_menu.vue'
...
if (window.mobilecheck()) {
  ...
  if (process.env.VUE_APP_ENV === 'LOCAL') {
    window.location.href = process.env.VUE_APP_LOCAL_MOBILE_URI + window.location.pathname + (param[1] ? '?' + param[1] : '')
  } else {
    window.location.href = process.env.VUE_APP_PRODC_MOBILE_URI + window.location.pathname + (param[1] ? '?' + param[1] : '')
  }
} else {
  new Vue({
    router,
