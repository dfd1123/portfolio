<template>
  <div>
    <header id="dg-find-hd">
      <h1 class="dg-find-hd-logo dg_blind">gonggle</h1>
      <div class="dg-find-hd-step_wrap">
        <h2>
          구독(정기)결제
          <button type="button" class="_back_btn" @click="$router.go(-2)">뒤로가기</button>
        </h2>
      </div>
    </header>
    <section id="subscribe-page" class="step_common">
      ...
      <form>
        <fieldset class="dg-reg_check_form">
          <div class="dg_write write_check" style="text-align: left;">
            ...
            <div class="reg_check_peice">
              ...
              <div class="dg-reg_agree">
                ...
                <a href="#" class="view_agree_desc display_none">보기</a>
              </div>
            </div>
          </div>
          ...
          this.subscribes.forEach(subscribe => {
            if (subscribe.choice) {
              if (subscribe.id === "bio") {
                const params = {
                  PCD_CPAY_VER: "1.0.1",
                  PCD_RST_URL: this.$APIURI + "payple/pay_mobile",
                  PCD_PAY_TYPE: type,
                  ...
                  PCD_USER_DEFINE1: subscribe.price,
                  PCD_USER_DEFINE2: this.$store.state.beforUrl,
