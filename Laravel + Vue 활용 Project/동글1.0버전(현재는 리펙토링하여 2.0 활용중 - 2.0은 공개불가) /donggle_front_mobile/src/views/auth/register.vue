    ...
    >
      ...
      <form>
        <fieldset class="dg-write_form">
          ...
          <div class="dg_write write_mail input_btn_with_textbox_wrap">
    ...
    methods: {
      ...
      async Validation () {
        ...
        if (this.form.password === '') {
          await this.WarningAlert('비밀번호를 입력해주세요!')
          this.$refs.password.focus()
          return false
        } else {
          if (!this.ChkPwd(this.form.password)) {
            await this.WarningAlert('비밀번호는 영문,숫자를 혼합하여 8~20자 이내로 입력하셔야 합니다!')
            this.$refs.password.focus()
            return false
          }
        }
        ...
        if (this.form.password_confirm === '') {
          await this.WarningAlert('비밀번호 재확인을 입력해주세요!')
      ...
      async Submit () {
        ...
        if (validation) {
          try {
            ...
            const response = await this.$http.post(this.$APIURI + 'register', this.form)
