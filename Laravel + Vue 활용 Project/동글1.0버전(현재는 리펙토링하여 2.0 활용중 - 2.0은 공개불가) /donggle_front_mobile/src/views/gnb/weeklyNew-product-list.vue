        ...
        <div class="l-con-article">
          <div class="l-contents-group">
            ...
            <div
              ...
              class="non_data"
            >
              주간신상(Weekly new)으로 등록된 상품이 아직 없습니다.
            </div>
          </div>
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    destroyed () {
      window.removeEventListener('scroll', this.InfiniteLoad, false)
      window.removeEventListener('scroll', this.OrderByScroll, false)
    },
    methods: {
      async ItemLoad () {
        try {
          ...
          const res = (await this.$http.get(this.$APIURI + 'items/week_list', { params })).data
