    ...
    <div class="dg-order-info-group clear_both">
      <div class="in-detail-info">
        ...
        <article class="l-con-article">
          <h4 class="in-subject">
            <span>취소사유</span>
            <span class="_etc_ment">제품이 택배사에 전달 된 후의 주문취소는 정상처리가 되지 않을 수
              있습니다.</span>
          </h4>
          ...
          <div class="dg-total-price-card">
            <ul class="in-price-infos">
              ...
              <li class="_col col--sale">
                ...
                <b class="_prc"><i>-{{ NumberFormat(orderLists[0]?orderLists[0].level_discount : 0) }}</i> 원</b>
              </li>
              <li class="_col col--sale">
              ...
              <li class="_col _col--total">
                <span class="_label">최종 취소금액</span>
    ...
    methods: {
          ...
          try {
            ...
            if (res.state === 1) {
              res.query.orders.forEach(order => {
                if (order.od_status === 'deposit_wait' || order.od_status === 'order_apply') {
      ...
      Validation () {
        if (!this.form.refund_reason || this.form.refund_reason === '') {
          this.WarningAlert('취소 사유를 선택해주세요!')
          return false
        }
      ...
      async Submit () {
        ...
        const params = this.form
        const result = await this.Confirm('정말 주문을 취소 하시겠습니까?')
        if (result) {
          if (this.$route.query.order_id) {
            try {
              this.$store.commit('ProgressShow')
              const res = (await this.$http.post(this.$APIURI + 'order/order_all_cancel', params)).data

              if (res.state === 1) {
                const reslt = await this.SuccessAlert('성공적으로 주문을 취소하였습니다.')
                if (reslt) {
            ...
            try {
              this.$store.commit('ProgressShow')
              const res = (await this.$http.post(this.$APIURI + 'order/order_part_cancel', params)).data
