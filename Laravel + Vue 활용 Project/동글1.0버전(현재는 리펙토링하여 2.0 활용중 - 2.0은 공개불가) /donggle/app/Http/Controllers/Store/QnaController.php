...
class QnaController extends Controller
{
    ...
    public function show(Request $request, $req)
    {
        switch($req){
            case 'list':
                ...
                ->where('type',1)->where('q_id', $uid)->limit($limit)->offset($offset)->orderBy('q_datetime','DESC')->get();
    ...
    public function store(Request $request)
    {
        ...
        $uid = Auth::guard('api')->id();
        $q_email = Auth::guard('api')->user()->email;
        $q_name = Auth::guard('api')->user()->name;
        $q_hp = Auth::guard('api')->user()->mobile_number;
    ...
    {   
        ...
        $delete = DB::table('qna')->where('id',$id)->where('q_id',$uid)->delete();
        ...
        if($delete){
            ...
            $this->res['query'] = $delete;
            $this->res['msg'] = "권한없거나 이미 삭제된 문의글입니다!";
            $this->res['state'] = config('res_code.QUERY_ERR');
        }
