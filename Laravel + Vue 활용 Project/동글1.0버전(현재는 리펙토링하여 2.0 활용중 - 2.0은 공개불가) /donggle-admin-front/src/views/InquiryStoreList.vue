<template>
  <div id="app">
    <layout-header title="스토어 문의 리스트" />

    <!-- contents -->
    <div id="admin-container">
      <div class="wrapper">
        <!-- 모바일에서 .fixed_top 를 주면 탭버튼과 셀렉트들이 탑에 고정됨 -->
        <div id="page-inquiry-store-list-wrap" class="check-fixed-top-mobile">
              ...
              <div class="table-area">
                ...
                <div class="show-pc">
                  <table-data-grid
                    ...
                    :table-classes="['inquiry-store-tbl']"
                  ...
                  >
                    ...
                    <template v-slot:rows="rows">
                      <tr v-for="(row, index) in rows.list" :key="row[rows.index]" class="in-tr">
                        ...
                        <td class="ta-left">{{row.subject || '-'}}</td>
  ...
  methods: {
    ...
    async searchClick () {
      try {
        ...
        const data = await this.$axios
          .get('/api/store/store_qna/store_qa', {
            params: {
              ...params,
        ...
        this.table.rows = (this.table.rows || []).concat(data.store_qna)
      } catch (e) {
        console.log(e)
    ...
    async selectedReadyButtonClick (rows, index) {
      try {
        ...
        await this.$axios
          .post('/api/store/qna/delete', formData, { headers })
          .then(this.normalOrError)
