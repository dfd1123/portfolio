<template>
  <div id="app">
    <layout-header title="취소/환불내역 리스트" />
    ...
    <div id="admin-container">
      <div class="wrapper">
        ...
        <div
          id="page-history-cancel-list-wrap"
                  ...
                  >
                    ...
                    <template v-slot:rows="rows">
                      ...
                      >
                        ...
                        <td>
                          <i>{{_get((row.cancel_date || '').split(','), 0) || '-'}}</i>
                        </td>
                        <td>
                          <a
                            href="#"
                        ...
                        >
                          ...
                          >
                            <div class="order-date clesarfix">
                              <div class="_cancel_desc">
                                취소신청일
                                <span class="_cancel_date">{{_get((row.receipt_time || '').split(','), 0) || '-'}}</span>
                              </div>
                              <span class="_date">{{row.receipt_time ? $moment(row.receipt_time).format('MM-DD') : '-'}}</span>
                              <span class="_order_num">{{row.order_id}}</span>
                              <p class="_stat">환불처리</p>
                            </div>
                  ...
                  >
                    <div class="popup-content-box">
                      ...
                      <div class="icon-search-btn-wrap">
                        <table-filter-button @click="searchButtonClick($event); popup.isVisible = false" />
                      </div>
                      <!-- popup footer E -->
                    </div>
                  </div>
    ...
    async searchClick () {
      try {
        ...
        const data = await this.$axios
          .get('/api/store/order/cancel_list', {
            params: {
              ...params,
