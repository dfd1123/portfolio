...
class FacadeLoader
{
    ...
    {
        if (file_exists($path = $this->cache_path('facade-'.sha1($alias).'.php'))) {
            return $path;
        }

        file_put_contents($path, $this->formatFacadeStub(
            $alias, file_get_contents($this->base_path('/stubs/facade.stub'))
        ));
    ...
    private function cache_path($path = '') {
        return base_path('storage/framework/cache/'.$path);
    }
